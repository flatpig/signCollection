    ...
    if (cookie) {
      ...
      if ($.shareCoseList.length >= 2) {
        for (let y = 0; y < $.shareCoseList.length; y++) {
          console.log(
            `京东账号${$.index} ${$.nickName || $.UserName}去助力${
              $.shareCoseList[y]
            }`
          );
          await helpShare({
...
async function main() {
  await superboxSupBoxHomePage({
    taskId: '',
    linkId: 'DQFdr1ttvWWzn0wsQ7JDZQ',
    encryptPin: ''
  });
  console.log(
    `【京东账号${$.index}】${$.nickName || $.UserName}互助码：${$.encryptPin}`
  );
  await $.wait(1000);
  await apTaskList({
    linkId: 'DQFdr1ttvWWzn0wsQ7JDZQ',
  ...
  if ($.allList) {
    for (let i = 0; i < $.allList.length; i++) {
      ...
      if (
        ['BROWSE_SHOP'].includes($.oneTask.taskType) &&
        $.oneTask.taskFinished === false
      ) {
        await apTaskDetail({
          taskId: $.oneTask.id,
        ...
        for (
          let y = 0;
          y < $.doList.status.finishNeed - $.doList.status.userFinishedTimes;
          y++
        ) {
    ...
    $.get(taskGetUrl('superboxSupBoxHomePage', body), (err, resp, data) => {
      try {
        if (err) {
          ...
          console.log(
            `${$.name} superboxSupBoxHomePage API请求失败，请检查网路重试`
          );
        } else {
    ...
    $.get(taskGetUrl('superboxOrdinaryLottery', body), (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(
            `${$.name} superboxOrdinaryLottery API请求失败，请检查网路重试`
          );
        } else {
          data = JSON.parse(data);
          //console.log(JSON.stringify(data));
          if (
            ...
            console.log(
              `开箱成功应该获得了空气${JSON.stringify(data.data)}\n\n`
            );
          } else {
...
function taskGetUrl(functionId, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${functionId}&body=${JSON.stringify(
      body
    )}&_t=${Date.now()}&appid=activities_platform&client=wh5&clientVersion=1.0.0`,
    ...
    headers: {
      ...
      Referer:
        'https://prodev.m.jd.com/mall/active/3z9BVbnAa1sVy88yEyKdp9wcWZ7Z/index.html?'
    }
  };
}
