...
const ACT_ID = 'dz2010100034444201',
  shareUuid = '28a699ac78d74aa3b31f7103597f8927';
let ADD_CART = false;
ADD_CART = $.isNode()
...
function accessLogWithAD() {
  return new Promise((resolve) => {
    let body = `venderId=${$.shopId}&code=99&pin=${encodeURIComponent(
      $.token
    )}&activityId=${ACT_ID}&pageUrl=https%3A%2F%2Flzdz-isv.isvjcloud.com%2Fdingzhi%2Fbook%2Fdevelop%2Factivity%3FactivityId%3Ddz2010100034444201%26lng%3D107.146945%26lat%3D33.255267%26sid%3Dcad74d1c843bd47422ae20cadf6fe5aw%26un_area%3D27_2442_2444_31912&subType=app&adSource=`;
    $.post(
      taskPostUrl('common/accessLogWithAD', body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${$.name} API请求失败，请检查网路重试`);
