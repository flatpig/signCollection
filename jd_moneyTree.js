/*
京东摇钱树 ：https://raw.githubusercontent.com/flatpig/signCollection/master/jd_moneyTree.js
更新时间：2020-11-03
京东摇钱树支持京东双账号
...
// [task_local]
// #京东摇钱树
// 3 */2 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_moneyTree.js, tag=京东摇钱树, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdyqs.png, enabled=true
// Loon
// [Script]
// cron "3 */2 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_moneyTree.js,tag=京东摇钱树
// Surge
//京东摇钱树 = type=cron,cronexp="3 */2 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_moneyTree.js
const $ = new Env('京东摇钱树');
...
async function jd_moneyTree() {
  ...
  console.log(
    `运行脚本次数和设置的次数是否相等::${
      $.getdata($.treeMsgTime) * 1 === Notice
    }`
  );
  jdNotify = $.getdata('jdMoneyTreeNotify')
          ...
          if (data) {
            ...
            if (res && res.resultCode === 0) {
              ...
              if (res.resultCode === 3) {
                ...
                $.msg(
                  $.name,
                  `【提示】京东账号${$.index}${UserName}cookie已失效,请重新登录获取`,
                  'https://bean.m.jd.com/',
                  { 'open-url': 'https://bean.m.jd.com/' }
                );
                if ($.index === 1) {
...
function harvest() {
  ...
  return new Promise((rs, rj) => {
    request('harvest', data).then((harvestRes) => {
      if (
        harvestRes &&
        harvestRes.resultCode === 0 &&
        harvestRes.resultData.code === '200'
      ) {
        console.log('收获金果');
        let data = harvestRes.resultData.data;
        message += `【距离${data.treeInfo.level + 1}级摇钱树还差】${
          data.treeInfo.progressLeft
        }\n`;
...
function signEveryDay() {
  return new Promise(async (resolve) => {
    ...
    if (signIndexRes.resultCode === 0) {
      console.log(
        `每日签到条件查询:${
          signIndexRes.resultData.data.canSign === 2
            ? '可以签到'
            : '已经签到过了'
        }`
      );
      if (
        signIndexRes.resultData &&
        signIndexRes.resultData.data.canSign == 2
      ) {
        console.log('准备每日签到');
        let signOneRes = await signOne(signIndexRes.resultData.data.signDay);
        console.log(
          `第${signIndexRes.resultData.data.signDay}日签到结果:${JSON.stringify(
            signOneRes
          )}`
        );
        if (signIndexRes.resultData.data.signDay === 7) {
          ...
          console.log(
            `店铺券（49-10）领取结果：${JSON.stringify(getSignAwardRes)}`
          );
          if (
            getSignAwardRes.resultCode === 0 &&
            getSignAwardRes.data.code === 0
          ) {
...
async function setUserLinkStatus(missionId) {
  ...
  do {
    ...
    console.log(
      `missionId为${missionId}：：第${
        index + 1
      }次浏览活动完成: ${JSON.stringify(response)}`
    );
    // if (resultCode === 0) {
    ...
    if (canSteal) {
      ...
      for (let item of $.friendRankList) {
        if (!item.self && item.steal) {
          ...
          const stealFruitRes = await stealFruit(
            item.encryPin,
            $.friendTree.stoleInfo
          );
          if (
            stealFruitRes &&
            stealFruitRes.resultCode === 0 &&
            stealFruitRes.resultData.code === '200'
          ) {
