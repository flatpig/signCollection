...
async function main(cookie) {
  ...
  let homeData = await takeRequest(
    '?functionId=travel_getHomeData',
    'functionId=travel_getHomeData&body={}&client=wh5&clientVersion=1.0.0',
    cookie
  );
  if (
    JSON.stringify(homeData) === '{}' ||
    homeData.bizCode !== 0 ||
    homeData.bizMsg !== 'success'
  ) {
  ...
  let qryCompositeMaterials = await takeRequest(
  ...
  if (
    ...
    qryCompositeMaterials.feedBottomData0 &&
    qryCompositeMaterials.feedBottomData0.list
  ) {
    await $.wait(2000);
    ...
    for (let i = 0; i < shopList.length && !max; i++) {
      ...
      if (
        shopInfo &&
        shopInfo.innerLink &&
        shopInfo.innerLink.match(/{\"appId\":\"(.*)\",\"category/) &&
        shopInfo.innerLink.match(/{\"appId\":\"(.*)\",\"category/)[1]
      ) {
        let appId = shopInfo.innerLink.match(
          /{\"appId\":\"(.*)\",\"category/
        )[1];
        ...
        if (!mainShopInfo.userInfo.attention) {
          ...
          shopody = `functionId=followShop&body=${encodeURIComponent(
            thisBody
          )}&t=${Date.now()}&eid=&appid=shop_view&clientVersion=10.0.0&client=wh5&uuid=8888`;
        ...
        for (let j = 0; j < taskList.length; j++) {
          ...
          if (oneTask.type === 5 || oneTask.type === 3) {
            ...
            let taskGoods = await takeRequest(
              '',
              `functionId=jm_goods_taskGoods&body=${encodeURIComponent(
                thisBody
              )}&t=${Date.now()}&eid=&appid=shop_view&clientVersion=10.0.0&client=wh5&uuid=8888`,
              cookie
            );
            ...
            for (let k = 0; k < skuList.length && k < needTime; k++) {
              ...
              await takeRequest(
                '',
                `functionId=jm_task_process&body=${encodeURIComponent(
                  thisBody
                )}&t=${Date.now()}&eid=&appid=shop_view&clientVersion=10.0.0&client=wh5&uuid=8888`,
                cookie
              );
              ...
              let finishInfo = await takeRequest(
                ...
                `functionId=jm_task_process&body=${encodeURIComponent(
                  thisBody
                )}&t=${Date.now()}&appid=shop_view&clientVersion=10.0.0&client=wh5&uuid=8888`,
                cookie
              );
