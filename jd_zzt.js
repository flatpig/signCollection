...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    $.raveldoubleeleven = `ravel-double-eleven=${
      authorizationInfo[$.UserName]
    }`;
    $.canHelp = true;
    for (let j = 0; j < codeList.length && $.canHelp; j++) {
      ...
      let helpInfo = await taskPostUrl(
        'invite',
        `inviter_id=${$.oneInfo.code}`
      );
      if (!helpInfo) {
        console.log(`助力成功`);
        $.oneInfo.needTime--;
...
async function main() {
  ...
  console.log(
    `昵称：${user.nickname},剩余金币值：${user.coins}，助力码：${user.id}\n`
  );
  ...
  if (state.friend.length !== 5) {
    codeList.push({
      user: $.UserName,
      code: user.id,
      needTime: 5 - state.friend.length
    });
  }
  ...
  for (let i = 0; i < time; i++) {
    ...
    if (dice.prize_info && JSON.stringify(dice.prize_info) !== '[]') {
      ...
      if (dice.prize_info.type === 3) {
        let linInfo = await taskPostUrl(
          'link',
          `id=${dice.prize_info.id}&token=${dice.prize_info.token}&link=${dice.prize_info.link}`
        );
        console.log(`点击跳转`);
        console.log(`获得：${linInfo.beans || 0}京豆`);
      }
    } else if (
      JSON.stringify(dice.prize_info) === '[]' ||
      dice.prize_info === false
    ) {
      ...
      try {
        if (url === 'invite') {
          ...
          if (err) {
            ...
            data = JSON.parse(data);
            if (
              (data.coins && url == 'product_view') ||
              url == 'shop_view' ||
              url == 'meetingplace_view'
            )
              console.log(
                `操作成功,获得${data.coins}金币，当前金币${data.user_coins}`
              );
            resolve(data);
          }
        }
