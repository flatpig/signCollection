...
[task_local]
#宠汪汪积分兑换奖品
0 0-16/8 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_reward.js, tag=宠汪汪积分兑换奖品, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true
// Loon
[Script]
cron "0 0-16/8 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_reward.js,tag=宠汪汪积分兑换奖品
// Surge
宠汪汪积分兑换奖品 = type=cron,cronexp="0 0-16/8 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_reward.js
 */
...
async function joyReward() {
  ...
  if ($.getExchangeRewardsRes && $.getExchangeRewardsRes.success) {
    ...
    if (
        ...
        if (leftStock) {
          ...
          if ($.exchangeRes && $.exchangeRes.success) {
            if ($.exchangeRes.errorCode === 'buy_success') {
              ...
              $.exchangeRes &&
              $.exchangeRes.errorCode === 'buy_limit'
            ) {
              console.log(
      ...
      console.log(
        '您设置了不兑换京豆,如需兑换京豆，请去BoxJs重新设置或修改第20行代码'
      );
    }
  } else {
