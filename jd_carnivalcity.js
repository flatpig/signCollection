...
async function doHotProducttask() {
  ...
  if ($.hotProductList && $.hotProductList.length)
    console.log(`开始 【浏览热销手机产品】任务,需等待6秒`);
      ...
      try {
        if (err) {
          ...
          if (data.code === 200) {
            ...
            for (let sku of $.shopTask.filter(
              (vo) => !!vo && vo['status'] !== '4'
            )) {
              console.log(
                `\n开始做 品牌手机 【${data['data']['brandName']}】 任务`
              ...
              console.log(
                `开始浏览 2-F 专柜区 任务 ${sku['name']}，需等待10秒`
              );
              await doBrowse(
                sku['id'],
                brandId,
            ...
            for (let sku of $.meetingTask.filter(
              (vo) => !!vo && vo['status'] !== '4'
            )) {
              console.log(
                `\n开始做 品牌手机 【${data['data']['brandName']}】 任务`
              ...
              console.log(
                `开始浏览 3-F 综合区 任务 ${sku['name']}，需等待10秒`
              );
              await doBrowse(
                sku['id'],
                brandId,
            ...
            if (
              $.questionTask.hasOwnProperty('id') &&
              $.questionTask['result'] === '0'
            ) {
              console.log(
                `\n开始做 品牌手机 【${data['data']['brandName']}】 任务`
              );
...
async function doBrowseshopTask() {
  ...
  if ($.browseshopList && $.browseshopList.length)
    console.log(`\n开始 【逛好货街，做任务】，需等待10秒`);
    ...
    $.get(taskUrl(body), async (err, resp, data) => {
      try {
        if (err) {
          ...
          if (data.code === 200) {
            console.log(
              `助力情况：${data['data']['supportedNums']}/${data['data']['supportNeedNums']}`
            );
            message += `邀请好友助力：${data['data']['supportedNums']}/${data['data']['supportNeedNums']}\n`;
          }
        }
            ...
            if (data.data && data.data.length) {
              for (let i = 0; i < data.data.length; i++) {
                ...
                if (data.data[i].status === '1') {
                  console.log(
                    `开始领取往期奖励【${data.data[i]['prizeName']}】`
                  );
                  ...
                  if (res && res.code === 200) {
                    ...
                    console.log(
                      `${data.data[i]['date']}日 【${res.data}】京豆奖励领取成功`
                    );
                  } else {
                  ...
                  console.log(
                    `${data.data[i]['date']}日 【${data.data[i]['prizeName']}】往期京豆奖励已领取~`
                  );
                } else {
                  console.log(
                    `${data.data[i]['date']}日 【${data.data[i]['status']}】往期京豆奖励，今日争取进入前30000名哦~`
                  );
                }
              }
            }
      ...
      try {
        if (err) {
          ...
          if (data.code === 200) {
            ...
            $.log(
              `【京东账号${$.index}（${$.UserName}）的${$.name}好友互助码】${data.data.shareId}\n\n`
            );
      ...
      try {
        if (err) {
          ...
          if (data.code === 200) {
            ...
            console.log(
              `开始抽奖，当前积分可抽奖${parseInt($.integralCount / 50)}次\n`
            );
...
function taskUrl(body = {}) {
  return {
    url: `${JD_API_HOST}?appid=guardian-starjd&functionId=carnivalcity_jd_prod&body=${JSON.stringify(
      body
...
async function showMsg() {
  if ($.beans) {
    allMessage += `京东账号${$.index} ${
      ...
      $.beans
    }京豆\n${message}活动地址："https://carnivalcity.m.jd.com/"${
