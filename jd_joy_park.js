      ...
      if ($.isNode()) {
        if (process.env.HELP_JOYPARK && process.env.HELP_JOYPARK == 'false') {
          ...
          if ($.kgw_invitePin && $.kgw_invitePin.length) {
            ...
            $.kgw_invitePin = [...($.kgw_invitePin || [])][
              Math.floor(Math.random() * $.kgw_invitePin.length)
            ];
...
function getJoyList(printLog = false) {
  ...
  return new Promise((resolve) => {
    $.get(
      taskGetClientActionUrl(
        `appid=activities_platform&body={"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}`,
        `joyList`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            if (printLog) {
              ...
              for (let i = 0; i < data.data.activityJoyList.length; i++) {
                //$.wait(50);
                $.log(
                  `id:${data.data.activityJoyList[i].id}|name: ${data.data.activityJoyList[i].name}|level: ${data.data.activityJoyList[i].level}`
                );
              ...
              for (let i = 0; i < data.data.workJoyInfoList.length; i++) {
                //$.wait(50)
                $.log(
                  `工位: ${data.data.workJoyInfoList[i].location} [${
                    data.data.workJoyInfoList[i].unlock ? `已开` : `未开`
                  }]|joy= ${
                    data.data.workJoyInfoList[i].joyDTO
                      ? `id:${data.data.workJoyInfoList[i].joyDTO.id}|name: ${data.data.workJoyInfoList[i].joyDTO.name}|level: ${data.data.workJoyInfoList[i].joyDTO.level}`
                      : `毛都没有`
                  }`
                );
              }
...
async function doJoyMoveUpAll(activityJoyList, workJoyInfoList) {
  let workJoyInfoUnlockList = workJoyInfoList.filter(
    (row) => row.unlock && row.joyDTO === null
  );
  if (activityJoyList.length !== 0 && workJoyInfoUnlockList.length !== 0) {
    let maxLevelJoy = Math.max.apply(
      Math,
      activityJoyList.map((o) => o.level)
    );
    let maxLevelJoyList = activityJoyList.filter(
      (row) => row.level === maxLevelJoy
    );
    $.log(
      `下地干活！ joyId= ${maxLevelJoyList[0].id} location= ${workJoyInfoUnlockList[0].location}`
    );
...
async function doJoyMoveDownAll(workJoyInfoList) {
  ...
  for (let i = 0; i < workJoyInfoList.length; i++) {
    ...
    if (workJoyInfoList[i].unlock && workJoyInfoList[i].joyDTO) {
      $.log(
        `从工位移除 => id:${workJoyInfoList[i].joyDTO.id}|name: ${workJoyInfoList[i].joyDTO.name}|level: ${workJoyInfoList[i].joyDTO.level}`
      );
...
async function doJoyMergeAll(activityJoyList) {
  ...
  if (joyMinLevelArr.length >= 2) {
    $.log(
      `开始合成 ${minLevel} ${joyMinLevelArr[0].id} <=> ${joyMinLevelArr[1].id} 【限流严重，5秒后合成！如失败会重试】`
    );
    await $.wait(5000);
    ...
    joyMinLevelArr.length === 1 &&
    joyMinLevelArr[0].level < fastBuyLevel
  ) {
    ...
    $.log(
      `现在最高可以购买: ${fastBuyLevel}  购买 ${fastBuyLevel} 的joy   你还有${joyBaseInfo.joyCoin}金币`
    );
...
function doJoyMove(joyId, location) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskGetClientActionUrl(
        `body={"joyId":${joyId},"location":${location},"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}&appid=activities_platform`,
        `joyMove`
      ),
      async (err, resp, data) => {
        try {
...
function doJoyMerge(joyId1, joyId2) {
  ...
  return new Promise((resolve) => {
    $.get(
      taskGetClientActionUrl(
        `body={"joyOneId":${joyId1},"joyTwoId":${joyId2},"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}&appid=activities_platform`,
        `joyMergeGet`
      ),
      async (err, resp, data) => {
        try {
...
async function doJoyBuy(level, activityJoyList) {
  ...
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"level":${level},"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}&appid=activities_platform`,
        `joyBuy`
      ),
      async (err, resp, data) => {
        try {
            ...
            switch (data.code) {
              ...
              case 518:
                ...
                if (activityJoyList) {
                  ...
                  await doJoyRecovery(
                    activityJoyList.filter((row) => row.level === minLevel)[0]
                      .id
                  );
                }
                break;
...
function doJoyRecovery(joyId) {
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"joyId":${joyId},"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}&appid=activities_platform`,
        `joyRecovery`
      ),
      async (err, resp, data) => {
        try {
...
function doJoyRestart() {
  return new Promise((resolve) => {
    $.post(
      taskPostClientActionUrl(
        `body={"linkId":"LsQNxL7iWDlXUs6cFl-AAg"}&appid=activities_platform`,
        `joyRestart`
      ),
      async (err, resp, data) => {
        try {
...
function taskGetClientActionUrl(body, functionId) {
  return {
    ...
    headers: {
      ...
      Origin: 'https://joypark.jd.com',
      Referer:
        'https://joypark.jd.com/?activityId=LsQNxL7iWDlXUs6cFl-AAg&lng=113.388006&lat=22.512549&sid=4d76080a9da10fbb31f5cd43396ed6cw&un_area=19_1657_52093_0',
      Cookie: cookie
    }
  };
}
