...
!(async () => {
  ...
  $.updatePkActivityIdRes = await getAuthorShareCode(
    'https://raw.githubusercontent.com/Aaron-lv/updateTeam/master/shareCodes/jd_cityShareCodes.json'
  );
  if (!$.updatePkActivityIdRes) {
    $.http
      .get({
        url: 'https://purge.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_cityShareCodes.json'
      })
      .then((resp) => {})
      .catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000);
    $.updatePkActivityIdRes = await getAuthorShareCode(
      'https://cdn.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/jd_cityShareCodes.json'
    );
  }
  await requireConfig();
...
function readShareCode() {
  ...
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `http://share.turinglabs.net/api/v3/carnivalcity/query/20/`,
        timeout: 20000
      },
      (err, resp, data) => {
        try {
