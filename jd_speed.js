/*
京东天天加速链接：https://raw.githubusercontent.com/flatpig/signCollection/master/jd_speed.js
更新时间：2020-11-03
支持京东双账号
...
// [task_local]
// #天天加速
// 8 */3 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_speed.js, tag=京东天天加速, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdjs.png, enabled=true
// Loon
// [Script]
// cron "8 */3 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_speed.js,tag=京东天天加速
// Surge
//天天加速 = type=cron,cronexp="8 */3 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_speed.js
const $ = new Env('✈️天天加速');
...
function jDSpeedUp(sourceId, doubleKey) {
  return new Promise((resolve) => {
    ...
    const url = {
      ...
      url: `${JD_API_HOST}?appid=memberTaskCenter&functionId=flyTask_${
        sourceId ? 'start' : 'state'
...
function spaceEventList() {
  return new Promise((resolve) => {
    ...
    const spaceEventUrl = {
      url: `${JD_API_HOST}?appid=memberTaskCenter&functionId=spaceEvent_list&body=${escape(
        JSON.stringify(body)
      )}`,
      headers: {
        Referer:
    ...
    if (spaceEventList && spaceEventList.length > 0) {
      ...
      for (let item of spaceEventList) {
        ...
        const spaceHandleUrl = {
          url: `${JD_API_HOST}?appid=memberTaskCenter&functionId=spaceEvent_handleEvent&body=${escape(
            JSON.stringify(body)
          )}`,
          headers: {
            Referer:
...
function energyPropList() {
  return new Promise((resolve) => {
    ...
    const QueryUrl = {
      ...
      url: `${JD_API_HOST}?appid=memberTaskCenter&functionId=energyProp_list&body=${escape(
        JSON.stringify(body)
      )}`,
      headers: {
        Referer:
    ...
    if (CID) {
      ...
      for (let i = 0; i < CID.length; i++) {
        ...
        const TUrl = {
          ...
          url: `${JD_API_HOST}?appid=memberTaskCenter&functionId=energyProp_gain&body=${escape(
            JSON.stringify(body)
          )}`,
          headers: {
            Referer:
...
function energyPropUsaleList(EID) {
  return new Promise((resolve) => {
    ...
    const EUrl = {
      ...
      url: `${JD_API_HOST}?appid=memberTaskCenter&functionId=energyProp_usalbeList&body=${escape(
        JSON.stringify(body)
      )}`,
      headers: {
        Referer:
    ...
    if (PropID) {
      for (let i = 0; i < PropID.length; i++) {
        ...
        const PropUrl = {
          ...
          url: `${JD_API_HOST}?appid=memberTaskCenter&functionId=energyProp_use&body=${escape(
            JSON.stringify(body)
          )}`,
          headers: {
            Referer:
