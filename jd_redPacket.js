...
[task_local]
#京东全民开红包
1 1 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_redPacket.js, tag=京东全民开红包, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jd_redPacket.png, enabled=true
Loon
[Script]
cron "1 1 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_redPacket.js, tag=京东全民开红包
Surge
京东全民开红包 = type=cron,cronexp=1 1 * * *,wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_redPacket.js
 */
    ...
    if (getTaskDetailForColorRes.data && getTaskDetailForColorRes.data.result) {
      ...
      for (let item of advertDetails) {
        ...
        if (item.id && item.status == 0) {
          let taskReportForColorRes = await taskReportForColor(
            taskType,
            item.id
          );
...
function taskUrl(function_id, body) {
  return {
    url: `${JD_API_HOST}?appid=jd_mp_h5&functionId=${function_id}&loginType=2&client=jd_mp_h5&t=${
