    ...
    if (cookiesArr[i]) {
      ...
      if ($.isNode()) {
        if (process.env.JOY_RUN_HELP_MYSELF) {
          ...
          let my_run_pins = [];
          Object.values(jdCookieNode)
            .filter(item => item.match(/pt_pin=([^; ]+)(?=;?)/))
            .map(item => my_run_pins.push(decodeURIComponent(item.match(/pt_pin=([^; ]+)(?=;?)/)[1])));
          ...
          Object.values(jdCookieNode)
            .filter(item => item.match(/pt_pin=([^; ]+)(?=;?)/))
            .map(item => run_pins.push(decodeURIComponent(item.match(/pt_pin=([^; ]+)(?=;?)/)[1])));
...
function enterRoom(invitePin) {
  return new Promise(resolve => {
    ...
    headers['Content-Type'] = 'application/json';
    const url =
      `https://draw.jdfcloud.com//common/pet/enterRoom/h5?reqSource=weapp&invitePin=${encodeURIComponent(invitePin)}&inviteSource=task_invite&shareSource=weapp&inviteTimeStamp=${Date.now()}&invokeKey=q8DNJdpcfRQ69gIx` +
      $.validate;
    const options = {
      url,
      body: JSON.stringify({}),
