/**
 ...
 surge
 [Script]
 宠汪汪邀请助力与赛跑助力 = type=cron,cronexp="15 10 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js
 宠汪汪助力更新Token = type=http-response,pattern=^https:\/\/draw\.jdfcloud\.com\/\/api\/user\/addUser\?code=\w+&, requires-body=1, max-size=0, script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js
 宠汪汪助力获取Token = type=http-request,pattern=^https:\/\/draw\.jdfcloud\.com\/\/api\/bean\/square\/silverBean\/task\/get\?, requires-body=1, max-size=0, script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js
 圈X
 [task_local]
 # 宠汪汪邀请助力与赛跑助力
 15 10 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js, tag=宠汪汪邀请助力与赛跑助力, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true
 [rewrite_local]
 # 宠汪汪助力更新Token
 ^https:\/\/draw\.jdfcloud\.com\/\/api\/user\/addUser\?code=\w+& url script-response-body https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js
 # 宠汪汪助力获取Token
 ^https:\/\/draw\.jdfcloud\.com\/\/api\/bean\/square\/silverBean\/task\/get\? url script-request-header https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js
 LOON：
 [Script]
 cron "15 10 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js, tag=宠汪汪邀请助力与赛跑助力
 http-response ^https:\/\/draw\.jdfcloud\.com\/\/api\/user\/addUser\?code=\w+& script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js, requires-body=true, timeout=10, tag=宠汪汪助力更新Token
 http-request ^https:\/\/draw\.jdfcloud\.com\/\/api\/bean\/square\/silverBean\/task\/get\? script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_run.js, requires-body=true, timeout=10, tag=宠汪汪助力获取Token
 **/
...
//给下面好友邀请助力的
let invite_pins = [
  'jd_6cd93e613b0e5,被折叠的记忆33,jd_704a2e5e28a66,jd_45a6b5953b15b,zooooo58',
];
//给下面好友赛跑助力
let run_pins = [
  'jd_6cd93e613b0e5,被折叠的记忆33,jd_704a2e5e28a66,jd_45a6b5953b15b,zooooo58',
...
const headers = {
  ...
  reqSource: 'weapp',
  'User-Agent':
    'Mozilla/5.0 (iPhone; CPU iPhone OS 13_6_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/7.0.15(0x17000f2d) NetType/4G Language/zh_CN',
  Cookie: '',
  openId: '',
...
function getToken() {
  ...
  if (
    isURL(
      url,
      /^https:\/\/draw\.jdfcloud\.com\/\/api\/user\/addUser\?code=\w+&/
    )
  ) {
    ...
    isURL(
      url,
      /^https:\/\/draw\.jdfcloud\.com\/\/api\/bean\/square\/silverBean\/task\/get\?/
    )
  ) {
...
async function invite(invite_pins) {
  console.log(
    `账号${
      $.index
    } [${UserName}] 给下面名单的人进行邀请助力\n${invite_pins.map((item) =>
      item.trim()
    )}\n`
  );
  for (let item of invite_pins.map((item) => item.trim())) {
    ...
    if (!data.success && data.errorCode === 'B0001') {
      ...
      if (helpStatus === 'help_full') {
        ...
        console.log(
          `助力失败，该好友 ${item} 已经满3人给他助力了,无需您再次助力\n`
        );
        continue;
...
function enterRoom(invitePin) {
  return new Promise((resolve) => {
    ...
    const options = {
      url: `${JD_BASE_API}/enterRoom?reqSource=weapp&invitePin=${encodeURI(
        invitePin
      )}`,
      headers,
    };
    ...
    $.get(options, (err, resp, data) => {
      try {
        if (err) {
          ...
          if (
            data.errorCode === 'help_ok' &&
            data.data.helpStatus === 'help_ok'
          ) {
            console.log(
...
function combatDetail(invitePin) {
  return new Promise((resolve) => {
    ...
    headers.LKYLToken = $.LKYLToken;
    const options = {
      url: `${JD_BASE_API}/combat/detail/v2?help=true&inviterPin=${encodeURI(
        invitePin
      )}`,
      headers,
    };
