  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      if ($.isNode()) {
        if (process.env.JOY_RUN_HELP_MYSELF) {
          ...
          Object.values(jdCookieNode)
            ...
            .map((item) =>
              my_run_pins.push(
                decodeURIComponent(item.match(/pt_pin=([^; ]+)(?=;?)/)[1])
              )
            );
          run_pins = [
            ...new Set(my_run_pins),
          ...
          fixPins.push(
            ...run_pins.splice(run_pins.indexOf('jd_61f1269fd3236'), 1)
          );
      ...
      new_invite_pins = [
        ...
        ...getRandomArrayElements(
          friendsArr,
          friendsArr.length >= 18 ? 18 : friendsArr.length
        )
      ];
...
async function getToken() {
  ...
  if (
    isURL(
      url,
      /^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/addUser\?code=/
    )
  ) {
    ...
    isURL(
      url,
      /^https:\/\/draw\.jdfcloud\.com(\/mirror)?\/\/api\/user\/user\/detail\?openId=/
    )
  ) {
...
function showMsg() {
  return new Promise(async (resolve) => {
    if ($.inviteReward || $.runReward) {
      let message = '';
      if ($.inviteReward > 0) {
        message += `给${$.inviteReward / 30}人邀请助力成功,获得${
          $.inviteReward
        }积分\n`;
      }
      if ($.runReward > 0) {
...
async function invite(invite_pins) {
  console.log(
    `账号${$.index} [${UserName}] 给下面名单的人进行邀请助力\n${invite_pins.map(
      (item) => item.trim()
    )}\n`
  );
...
function enterRoom(invitePin) {
  return new Promise((resolve) => {
    ...
    let opt = {
      ...
      url: `//draw.jdfcloud.com/common/pet/enterRoom/h5?reqSource=h5&invitePin=${encodeURI(
        invitePin
      )}&inviteSource=task_invite&shareSource=weapp&inviteTimeStamp=${Date.now()}&invokeKey=qRKHmL4sna8ZOP9F`,
      method: 'GET',
      data: {},
      credentials: 'include',
...
function helpInviteFriend(friendPin) {
  return new Promise((resolve) => {
    ...
    let opt = {
      // url: "//jdjoy.jd.com/common/pet/getPetTaskConfig?reqSource=h5",
      url: `//draw.jdfcloud.com/common/pet/helpFriend?friendPin=${encodeURI(
        friendPin
      )}&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`,
      method: 'GET',
      data: {},
      credentials: 'include',
      header: { 'content-type': 'application/json' }
...
async function run(run_pins) {
  console.log(
    `账号${$.index} [${UserName}] 给下面名单的人进行赛跑助力\n${run_pins.map(
      (item) => item.trim()
    )}\n`
  );
...
function combatHelp(friendPin) {
  return new Promise((resolve) => {
    ...
    let opt = {
      // url: "//jdjoy.jd.com/common/pet/getPetTaskConfig?reqSource=h5",
      url: `//draw.jdfcloud.com//common/pet/combat/help?friendPin=${encodeURI(
        friendPin
...
function combatDetail(invitePin) {
  return new Promise((resolve) => {
    ...
    let opt = {
      // url: "//jdjoy.jd.com/common/pet/getPetTaskConfig?reqSource=h5",
      url: `//draw.jdfcloud.com/common/pet/combat/detail/v2?help=true&inviterPin=${encodeURI(
        invitePin
      )}&reqSource=h5&invokeKey=qRKHmL4sna8ZOP9F`,
      method: 'GET',
      data: {},
  ...
  var _a = (function getTimeSign(e) {
    ...
    return (
      (h =
        'get' !== p &&
        ('post' !== p ||
          ('application/x-www-form-urlencoded' !== f.toLowerCase() &&
            i &&
            Object.keys(i).length))
          ? _o.Md5encode(
              _o.Base64Encode(_o.AesEncrypt('' + JSON.stringify(c(i)))) +
                '_' +
                g +
                '_' +
                u
