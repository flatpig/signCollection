...
支持京东双账号
京小超兑换奖品请使用此脚本 https://raw.githubusercontent.com/flatpig/signCollection/master/jd_blueCoin.js
脚本兼容: QuantumultX, Surge, Loon, JSBox, Node.js
// QuantumultX
[task_local]
#京小超
11 1-23/5 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_superMarket.js, tag=京小超, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxc.png, enabled=true
// Loon
[Script]
cron "11 1-23/5 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_superMarket.js,tag=京小超
// Surge
京小超 = type=cron,cronexp="11 1-23/5 * * *",wake-system=1,timeout=320,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_superMarket.js
 */
const $ = new Env('京小超');
...
function showMsg() {
  $.log(`\n${message}\n`);
  jdNotify = $.getdata('jdSuperMarketNotify')
    ? $.getdata('jdSuperMarketNotify')
    : jdNotify;
...
async function drawLottery() {
  ...
  drawLotteryFlag = $.getdata('jdSuperMarketLottery')
    ? $.getdata('jdSuperMarketLottery')
    : drawLotteryFlag;
  ...
  if (`${drawLotteryFlag}` === 'true') {
    ...
    if (smtg_lotteryIndexRes && smtg_lotteryIndexRes.data.bizCode === 0) {
      ...
      if (result.blueCoins > result.costCoins && result.remainedDrawTimes > 0) {
        ...
        console.log(
          `\n花费${result.costCoins}蓝币抽奖结果${JSON.stringify(
            drawLotteryRes
          )}`
        );
        await drawLottery();
    ...
    for (let item of taskList) {
      console.log(
        ` ${
          ...
          item['finishNum'] === item['targetNum'] ? '已完成' : '未完成'
        } ${item['finishNum']}/${item['targetNum']}`
      );
    }
    for (let item of taskList) {
      //领奖
      ...
      if (
        (item.type === 8 || item.type === 2 || item.type === 10) &&
        item.taskStatus === 0
      ) {
        await doDailyTask();
      }
...
function receiveBlueCoin(timeout = 0) {
  return new Promise((resolve) => {
    setTimeout(() => {
      $.get(
        taskUrl('smtg_receiveCoin', { type: 2 }),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log('\n京小超: API查询请求失败 ‼️‼️');
              ...
              if ($.data.data.bizCode === 0) {
                ...
                $.blueCionTimes++;
                console.log(
                  `【京东账号${$.index}】${$.nickName} 第${$.blueCionTimes}次领蓝币成功，获得${$.data.data.result.receivedBlue}个\n`
                );
...
async function businessCircleActivity() {
  ...
  if (
    ...
    smtg_getTeamPkDetailInfoRes.data.bizCode === 0
  ) {
    const {
      joinStatus,
      pkStatus,
    ...
    console.log(
      `PK队伍名称: [${currentUserPkInfo && currentUserPkInfo.teamName}]`
    );
    ...
    if (joinStatus === 0) {
      ...
      if (
        ...
        $.updatePkActivityIdRes.pkActivityId === pkActivityId
      ) {
        let Teams = [
          {
        ...
        const res = await smtg_joinPkTeam(
          Teams[randomNum].teamId,
          Teams[randomNum].inviteCode,
          pkActivityId
        );
      ...
      console.log(
        `对方战队战队 [${pkUserPkInfo.teamName}]/【${pkUserPkInfo.teamCount}】\n`
      );
    }
    if (pkStatus === 1) {
      console.log(`商圈PK进行中`);
      ...
      if (prizeInfo.pkPrizeStatus === 2) {
        ...
        console.log(
          `商圈PK奖励领取结果：${JSON.stringify(receivedPkTeamPrize)}`
        );
  ...
  if (
    businessCirclePKDetailRes &&
    businessCirclePKDetailRes.data.bizCode === 0
  ) {
    const {
      businessCircleVO,
    ...
    if (pkPrizeStatus === 2) {
      ...
      if (getPkPrizeRes.data.bizCode === 0) {
        ...
        message += `【商圈PK奖励】${
          pkPersonPrizeInfoVO.blueCoin + pkTeamPrizeInfoVO.blueCoin
        }蓝币领取成功\n`;
      }
    }
    console.log(
      `我方商圈人气值/对方商圈人气值：${businessCircleVO.hotPoint}/${otherBusinessCircleVO.hotPoint}`
    );
    console.log(
      `我方商圈成员数量/对方商圈成员数量：${businessCircleVO.memberCount}/${otherBusinessCircleVO.memberCount}`
    );
    ...
    businessCircleJump = $.getdata('jdBusinessCircleJump')
      ? $.getdata('jdBusinessCircleJump')
      : businessCircleJump;
    ...
    if (
      otherBusinessCircleVO.hotPoint - businessCircleVO.hotPoint > 300 &&
      Date.now() > pkSettleTime - 24 * 60 * 60 * 1000
    ) {
      //退出该商圈
      if (inviteCode === '-4msulYas0O2JsRhE-2TA5XZmBQ') return;
      console.log(
        `商圈PK已过1天，对方商圈人气值还大于我方商圈人气值300，退出该商圈重新加入`
      );
      await smtg_quitBusinessCircle();
    } else if (
      otherBusinessCircleVO.hotPoint > businessCircleVO.hotPoint &&
      Date.now() > pkSettleTime - 24 * 60 * 60 * 1000 * 2
    ) {
      //退出该商圈
      if (inviteCode === '-4msulYas0O2JsRhE-2TA5XZmBQ') return;
      console.log(
        `商圈PK已过2天，对方商圈人气值还大于我方商圈人气值，退出该商圈重新加入`
      );
      await smtg_quitBusinessCircle();
    }
  } else if (
    businessCirclePKDetailRes &&
    businessCirclePKDetailRes.data.bizCode === 222
  ) {
    ...
    businessCirclePKDetailRes &&
    businessCirclePKDetailRes.data.bizCode === 206
  ) {
    ...
    if (joinBusinessCircleRes.data.bizCode !== 0) {
      ...
      if (BusinessCircleList.data.bizCode === 0) {
        ...
        const { circleId } = businessCircleVOList[
          randomFriendPin(0, businessCircleVOList.length - 1)
        ];
        const joinBusinessCircleRes = await smtg_joinBusinessCircle(circleId);
        console.log(
          `随机加入商圈结果：${JSON.stringify(joinBusinessCircleRes)}`
        );
      }
    }
  } else {
    console.log(
      `访问商圈详情失败：${JSON.stringify(businessCirclePKDetailRes)}`
    );
  }
}
//我的货架
      ...
      if (item.groundStatus === 1) {
        ...
        if (productListRes.data.bizCode === 0) {
          ...
          if (productList && productList.length > 0) {
            ...
            console.log('无可上架产品');
            await unlockProductByCategory(
              item.shelfId.split('-')[item.shelfId.split('-').length - 1]
            );
          }
        }
        ...
        if (item.productInfo.productType === 2) {
          ...
          console.log(
            `[${item.name}][productType:${item.productInfo.productType}]`
          );
        }
      }
    }
...
async function unlockProductByCategory(category) {
  ...
  if (smtgProductListRes.data.bizCode === 0) {
    ...
    for (let item of productList) {
      if (
        item['unlockStatus'] === 1 &&
        item['shelfCategory'].toString() === category
      ) {
    ...
    if (productListByCategory && productListByCategory.length > 0) {
      ...
      await smtg_unlockProduct(
        productListByCategory[productListByCategory.length - 1]['productId']
      );
    } else {
...
async function upgrade() {
  superMarketUpgrade = $.getdata('jdSuperMarketUpgrade')
    ? $.getdata('jdSuperMarketUpgrade')
    : superMarketUpgrade;
  ...
  console.log(
    '目前没有平稳升级,只取倒数几个商品进行升级,普通货架取倒数4个商品,冰柜货架取倒数3个商品,水果货架取倒数2个商品'
  );
  const smtgProductListRes = await smtg_productList();
  if (smtgProductListRes.data.bizCode === 0) {
    let productType1 = [],
    ...
    const shelfCategorys = shelfCategory_1
      .concat(shelfCategory_2)
      .concat(shelfCategory_3);
    console.log(
    ...
    for (let item of shelfCategorys) {
      console.log(
        `  ${
          item['name'].length < 3 ? item['name'] + `\xa0` : item['name']
        }       ${
          item['shelfCategory'] === 1
            ? '普通货架'
            : item['shelfCategory'] === 2
            ? '冰柜货架'
            : item['shelfCategory'] === 3
            ? '水果货架'
            : '未知货架'
        }       ${
          item['unlockStatus'] === 0 ? '---' : item['level'] + '级'
        }     ${item['unlockStatus'] === 0 ? '未解锁' : '已解锁'}      ${
  ...
  if (shelfListRes.data.bizCode === 0) {
    ...
    if (shelfList_upgrade && shelfList_upgrade.length > 0) {
      ...
      for (let item of shelfList_upgrade) {
        console.log(
          ` [${item['name']}]         ${item['level']}/${item['maxLevel']}         ${item['upgradeCostGold']}`
        );
      }
      console.log(
        `开始升级[${shelfList_upgrade[0].name}]货架，当前等级${shelfList_upgrade[0].level}，所需金币${shelfList_upgrade[0].upgradeCostGold}\n`
      );
      ...
      if (productListRes.data.bizCode === 0) {
        ...
        if (productList2 && productList2.length > 0) {
          ...
          if (productNow && productNow.length > 0) {
            if (
              productList2.slice(-1)[0]['productId'] ===
              productNow[0]['productId']
            ) {
              console.log(
                `货架[${item.shelfId}]${productNow[0]['name']}已上架\n`
              );
              continue;
            }
          }
      ...
      for (let item2 of productList2) {
        ...
        if (shelfListRes.data.bizCode === 0) {
          ...
          for (let item3 of shelfList) {
            if (
              item3['shelfCategory'] === shelfCategory &&
              (item3['groundStatus'] === 1 || item3['groundStatus'] === 2)
            ) {
              shelfList2.push(item3['shelfId']);
            }
          }
          if (shelfList2 && shelfList2.length > 0) {
            const groundRes = await smtg_ground(
              item2['productId'],
              shelfList2.slice(-1)[0]
            );
...
function updatePkActivityId(
  url = 'https://raw.githubusercontent.com/lxk0301/updateTeam/master/jd_updateTeam.json'
) {
  return new Promise((resolve) => {
...
function updatePkActivityIdCDN(
  url = 'https://cdn.jsdelivr.net/gh/lxk0301/updateTeam@master/jd_updateTeam.json'
) {
  return new Promise((resolve) => {
    //https://cdn.jsdelivr.net/gh/lxk0301/updateTeam@master/jd_updateTeam.json
    //https://raw.githubusercontent.com/lxk0301/updateTeam/master/jd_updateTeam.json
              ...
              for (let item of taskList) {
                if (item.taskStatus === 1) {
                  if (item.prizeStatus === 1) {
                    ...
                    console.log(
                      `[${item.title}] 已做完 ${item.finishNum}/${item.targetNum}`
                    );
                  }
                } else {
                  console.log(
                    `[${item.title}] 未做完 ${item.finishNum}/${item.targetNum}`
                  );
                  if (item.content) {
...
function smtgDoAssistPkTask(code) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('smtg_doAssistPkTask', { inviteCode: code }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京小超: API查询请求失败 ‼️‼️');
...
function smtgObtainPkTaskPrize(taskId) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('smtg_obtainPkTaskPrize', { taskId: taskId }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京小超: API查询请求失败 ‼️‼️');
...
function smtgDoPkTask(taskId, itemId) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('smtg_doPkTask', { taskId: taskId, itemId: itemId }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京小超: API查询请求失败 ‼️‼️');
...
function smtg_joinBusinessCircle(circleId) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('smtg_joinBusinessCircle', { circleId }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京小超: API查询请求失败 ‼️‼️');
...
function smtg_receivedPkTeamPrize() {
  return new Promise((resolve) => {
    $.get(
      taskUrl('smtg_receivedPkTeamPrize', { channel: '1' }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京小超: API查询请求失败 ‼️‼️');
...
function smtg_lotteryIndex() {
  return new Promise((resolve) => {
    $.get(
      taskUrl('smtg_lotteryIndex', { costType: 1, channel: 1 }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京小超: API查询请求失败 ‼️‼️');
...
function smtg_drawLottery() {
  return new Promise(async (resolve) => {
    await $.wait(1000);
    $.get(
      taskUrl('smtg_drawLottery', { costType: 1, channel: 1 }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京小超: API查询请求失败 ‼️‼️');
...
function shareCodesFormat() {
  return new Promise((resolve) => {
    console.log(
      `第${$.index}个京东账号的助力码:::${jdSuperMarketShareArr[$.index - 1]}`
    );
    if (jdSuperMarketShareArr[$.index - 1]) {
      newShareCodes = jdSuperMarketShareArr[$.index - 1].split('@');
    } else {
      console.log(
        `由于您未提供与京京东账号相对应的shareCode,下面助力将采纳本脚本自带的助力码\n`
      );
      const tempIndex =
    ...
    console.log(
      `格式化后第${$.index}个京东账号的助力码${JSON.stringify(newShareCodes)}`
    );
    resolve();
  });
}
function requireConfig() {
  return new Promise((resolve) => {
    ...
    const jdShareCodes = $.isNode()
      ? require('./jdSuperMarketShareCodes.js')
      : '';
    //IOS等用户直接用NobyDa的jd cookie
    if ($.isNode()) {
    ...
    if ($.isNode()) {
      ...
      const boxShareCodeArr = [
        'jd_supermarket1',
        'jd_supermarket2',
        'jd_supermarket3',
      ];
      const boxShareCodeArr2 = [
        'jd2_supermarket1',
        'jd2_supermarket2',
        'jd2_supermarket3',
      ];
...
function taskUrl(function_id, body = {}) {
  return {
    ...
    headers: {
      'User-Agent':
        'jdapp;iPhone;9.0.8;13.6;Mozilla/5.0 (iPhone; CPU iPhone OS 13_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1',
      Host: 'api.m.jd.com',
      Cookie: cookie,
