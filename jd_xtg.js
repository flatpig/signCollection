 ...
 */
/**
 星推官脚本 https://raw.githubusercontent.com/flatpig/signCollection/master/jd_xtg.js
 ...
 [task_local]
 #京东星推官
 2 0 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_xtg.js, tag=京东星推官, enabled=true
 // Loon
 [Script]
 cron "2 0 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_xtg.js,tag=京东星推官
 // Surge
 京东星推官 = type=cron,cronexp=2 0 * * *,wake-system=1,timeout=320,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_xtg.js
 */
const $ = new Env('京东星推官');
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      ...
      console.log(
        `一共${starID.length}个${$.name}任务，耗时会很久，请提前知晓`
      );
...
async function showMsg() {
  ...
  if (nowTime > new Date(activeEndTime).getTime()) {
    ...
    $.msg(
      ...
      `做任务之前京豆总计:${$.beanCount}\n做完任务后京豆总计:${$.jdNum}\n${
        $.jdNum - $.beanCount > 0
          ? `获得京豆：${$.jdNum - $.beanCount}京豆 🐶(仅供参考)\n`
          : ''
      }京豆先到先得\n活动地址点击弹窗跳转后即可查看\n注：如未获得京豆就是已被分完`,
      {
        'open-url':
          'https://prodev.m.jd.com/mall/active/3gSzKSnvrrhYushciUpzHcDnkYE3/index.html',
      }
    );
    if ($.isNode())
      await notify.sendNotify(
