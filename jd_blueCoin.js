/*
京小超兑换奖品 脚本地址：https://raw.githubusercontent.com/flatpig/signCollection/master/jd_blueCoin.js
感谢@yangtingxiao提供PR
更新时间：2020-11-05
支持京东多个账号
...
[task_local]
#京小超兑换奖品
0 0 0 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_blueCoin.js, tag=京小超兑换奖品, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jxc.png, enabled=true
====================Loon=================
[Script]
cron "0 0 0 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_blueCoin.js,tag=京小超兑换奖品
===================Surge==================
京小超兑换奖品 = type=cron,cronexp="0 0 0 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_blueCoin.js
 */
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    if (cookie) {
      ...
      if (`${coinToBeans}` !== '0') {
        ...
        console.log(
          '查询到您设置的是不兑换京豆选项，现在为您跳过兑换京豆。如需兑换，请去BoxJs设置或者修改脚本coinToBeans\n'
        );
      }
      await msgShow();
    }
            ...
            if (data.data.bizCode === 0) {
              ...
              if (`${coinToBeans}` === '1000') {
                if (prizeList[1].beanType === 'BeanPackage') {
                  console.log(
                    `查询换${prizeList[1].title}ID成功，ID:${prizeList[1].prizeId}`
                  );
                ...
                if ($.totalBlue > $.blueCost) {
                  ...
                  console.log(
                    `兑换失败,您目前蓝币${$.totalBlue}个,不足以兑换${$.title}所需的${$.blueCost}个`
                  );
                ...
                if (prizeList[0].beanType === 'Bean') {
                  console.log(
                    `查询换${prizeList[0].title}ID成功，ID:${prizeList[0].prizeId}`
                  );
                ...
                if (prizeId) {
                  if (prizeList[i].inStock === 506) {
                    console.log(
                      `失败，您输入设置的${coinToBeans}领光了，请明天再来`
                    );
                  ...
                  console.log(
                    `奖品兑换列表[${coinToBeans}]已下架，请检查APP是否存在此商品，如存在请检查您的输入是否正确`
                  );
          ...
          if (safeGet(data)) {
            ...
            if ($.data.data.bizCode === 0) {
              if (`${coinToBeans}` === '1000') {
                $.beanscount++;
                console.log(
                  `【京东账号${$.index}】${$.nickName} 第${$.data.data.result.exchangeNum}次换${$.title}成功`
                );
                ...
                if (
                  $.data.data.result.exchangeNum === 20 ||
                  $.beanscount === coinToBeans ||
                  $.data.data.result.blue < 500
                )
                  return;
              } else {
                $.beanscount++;
...
async function msgShow() {
  ...
  $.log(
    `\n【京东账号${$.index}】${$.nickName}\n${
      coinToBeans
        ? `【兑换${$.title}】${$.beanscount ? `成功` : $.beanerr}`
        : '您设置的是不兑换奖品'
    }\n`
  );
  let ctrTemp;
  ...
  if ($.beanscount && ctrTemp) {
    $.msg(
      ...
      `【京东账号${$.index}】${$.nickName}\n${
        coinToBeans
          ? `【兑换${$.title}】${
              $.beanscount ? `成功，数量：${$.beanscount}个` : $.beanerr
            }`
          : '您设置的是不兑换奖品'
      }`
    );
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}&functionId=${function_id}&clientVersion=8.0.0&client=m&body=${escape(
      JSON.stringify(body)
    )}&t=${Date.now()}`,
    headers: {
      'User-Agent':
