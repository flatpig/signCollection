/*
东东水果:脚本更新地址 https://raw.githubusercontent.com/flatpig/signCollection/master/jd_fruit.js
更新时间：2020-11-04
...
[task_local]
#jd免费水果
5 6-18/6 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_fruit.js, tag=东东农场, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdnc.png, enabled=true
=========================Loon=============================
[Script]
cron "5 6-18/6 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_fruit.js,tag=东东农场

=========================Surge============================
东东农场 = type=cron,cronexp="5 6-18/6 * * *",wake-system=1,timeout=120,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_fruit.js

=========================小火箭===========================
东东农场 = type=cron,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_fruit.js, cronexpr="5 6-18/6 * * *", timeout=200, enable=true
...
async function doTenWaterAgain() {
  ...
  if (totalEnergy >= 100 && $.myCardInfoRes.doubleCard > 0) {
    ...
    for (
      let i = 0;
      i < new Array($.myCardInfoRes.doubleCard).fill('').length;
      i++
    ) {
...
async function receiveFriendInvite() {
  for (let code of newShareCodes) {
    ...
    console.log(
      `接收邀请成为好友结果:${JSON.stringify($.inviteFriendRes.helpResult)}`
    );
    if ($.inviteFriendRes.helpResult.code === '0') {
      console.log(
        `您已成为${$.inviteFriendRes.helpResult.masterUserInfo.nickName}的好友`
      );
...
function readShareCode() {
  return new Promise((resolve) => {
    $.get(
      { url: `http://api.turinglabs.net/api/v1/jd/farm/read/${randomCount}/` },
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
