...
// [task_local]
// #宠汪汪偷好友积分与狗粮
// 0 0,6 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_steal.js, tag=宠汪汪偷好友积分与狗粮, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true
// Loon
// [Script]
// cron "0 0,6 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_steal.js,tag=宠汪汪偷好友积分与狗粮
// Surge
// 宠汪汪偷好友积分与狗粮 = type=cron,cronexp="0 0,6 * * *",wake-system=1,timeout=320,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_steal.js
...
async function jdJoySteal() {
  ...
  if ($.getFriendsData && $.getFriendsData.success) {
    ...
    if (
      $.getFriendsData &&
      $.getFriendsData.datas &&
      $.getFriendsData.datas.length > 0
    ) {
...
async function stealFriendsFood() {
  ...
  if (jdJoyStealCoinTemp) {
    ...
    for (let friends of $.allFriends) {
      ...
      console.log(
        `stealFriendsFood---好友【${friendPin}】--偷食状态：${stealStatus}\n`
      );
...
async function helpFriendsFeed() {
  if ($.help_feed !== 200) {
    ...
    if (ctrTemp) {
      ...
      for (let friends of $.allFriends) {
        ...
        if (status === 'not_feed') {
          ...
          if (
            ...
            helpFeedRes &&
            helpFeedRes.errorCode === 'food_insufficient'
          ) {
      ...
      console.log(
        '您已设置不为好友喂食，现在跳过喂食，如需为好友喂食请在BoxJs打开喂食开关或者更改脚本 jdJoyHelpFeed 处'
      );
    }
  } else {
          ...
          if (data) {
            ...
            if (data.datas && data.datas.length > 0) {
              ...
              for (let item of data.datas) {
                if (
                  $.time('yyyy-MM-dd') === timeFormat(item.createdDate) &&
                  item.changeEvent === 'help_feed'
                ) {
                  $.help_feed = item.changeCoin;
                }
                if (
                  $.time('yyyy-MM-dd') === timeFormat(item.createdDate) &&
                  item.changeEvent === 'visit_friend'
                ) {
...
function showMsg() {
  $.stealFood =
    $.stealFood >= 0
      ? `【偷好友狗粮】获取${$.stealFood}g狗粮\n`
      : `【偷好友狗粮】${$.stealFood}\n`;
  $.stealFriendCoin =
    $.stealFriendCoin >= 0
      ? `【领取好友积分】获得${$.stealFriendCoin}个\n`
      : `【领取好友积分】${$.stealFriendCoin}\n`;
  $.helpFood =
    $.helpFood >= 0
      ? `【给好友喂食】消耗${$.helpFood}g狗粮,获得积分${$.helpFood}个\n`
      : `【给好友喂食】${$.helpFood}\n`;
