    ...
    for (let i = 0; i < getTaskList.length; i++) {
      ...
      if (oneTask.type === 'JOIN_SHOPPING_CART') {
        let getReward = await takeRequest(
          cookie,
          `appid=china-joy&functionId=champion_game_prod&body={"parentId":"${oneTask.parentId}","taskId":"${oneTask.taskId}","activityDate":"${
      ...
      if (oneTask.type === 'BROWSE_TASK' || oneTask.type === 'FOLLOW_CHANNEL_TASK') {
        ...
        let getReward = await takeRequest(
          cookie,
          `appid=china-joy&functionId=champion_game_prod&body={"parentId":"${oneTask.parentId}","taskId":"${oneTask.taskId}","timeStamp":${doInfo.timeStamp},"activityDate":"${
            homePage.activityDate
          }","apiMapping":"/api/task/getReward"}&t=${Date.now()}&loginType=2`,
        );
        console.log(`${userName},执行结果：${JSON.stringify(getReward)}`);
  ...
  if (answerCode) {
    if (homePage.userAnswerCode === null) {
      ...
      let guessAnswer = await takeRequest(
        cookie,
        `appid=china-joy&functionId=champion_game_prod&body={"activityDate":"${homePage.activityDate}","answerCode":"${answerCode}","apiMapping":"/api/guessAnswer"}&t=${Date.now()}&loginType=2`,
      );
