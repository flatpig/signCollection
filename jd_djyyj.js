  ...
  if (thisCode) {
    if (homePage.userAnswerCode === null) {
      ...
      let guessAnswer = await takeRequest(
        cookie,
        `appid=china-joy&functionId=champion_game_prod&body={"activityDate":"${homePage.activityDate}","answerCode":"${thisCode}","apiMapping":"/api/guessAnswer"}&t=${Date.now()}&loginType=2`,
      );
