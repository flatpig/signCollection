...
let cookiesArr = [];
// 环境变量：ANSWERCODE, 选择哪一个队伍,默认随机； 例：ANSWERCODE="A" 选择第一个队伍，ANSWERCODE="B" 选择第二个队伍
let answerCode = $.isNode()
  ? process.env.ANSWERCODE
    ? process.env.ANSWERCODE
    : `999`
  : `999`;
if ($.isNode()) {
    ...
    for (let j = 0; j < allShareList.length && canHelp && helpTime < 5; j++) {
      ...
      let doSupport = await takeRequest(
        ...
        `appid=china-joy&functionId=champion_game_prod&body={"shareId":"${
          oneCodeInfo.code
        }","apiMapping":"/api/doSupport"}&t=${Date.now()}&loginType=2`
      );
...
async function main(cookie) {
  ...
  await takeRequest(
    cookie,
    `appid=china-joy&functionId=champion_game_prod&body={"apiMapping":"/api/activityRules"}&t=${Date.now()}&loginType=2`
  );
  let homePage = await takeRequest(
    cookie,
    `appid=china-joy&functionId=champion_game_prod&body={"apiMapping":"/api/homePage"}&t=${Date.now()}&loginType=2`
  );
  let getTaskList = await takeRequest(
    cookie,
    `appid=china-joy&functionId=champion_game_prod&body={"apiMapping":"/api/task/getTaskList"}&t=${Date.now()}&loginType=2`
  );
  await takeRequest(
    cookie,
    `appid=china-joy&functionId=champion_game_prod&body={"apiMapping":"/api/isRisk"}&t=${Date.now()}&loginType=2`
  );
  if (
    JSON.stringify(homePage) === '{}' ||
    JSON.stringify(getTaskList) === '{}'
  ) {
  ...
  do {
    ...
    for (let i = 0; i < getTaskList.length; i++) {
      ...
      if (oneTask.type === 'JOIN_SHOPPING_CART') {
        let getReward = await takeRequest(
          cookie,
          `appid=china-joy&functionId=champion_game_prod&body={"parentId":"${
            oneTask.parentId
          }","taskId":"${oneTask.taskId}","activityDate":"${
            homePage.activityDate
      ...
      if (
        oneTask.type === 'BROWSE_TASK' ||
        oneTask.type === 'FOLLOW_CHANNEL_TASK'
      ) {
        ...
        let getReward = await takeRequest(
          ...
          `appid=china-joy&functionId=champion_game_prod&body={"parentId":"${
            oneTask.parentId
          }","taskId":"${oneTask.taskId}","timeStamp":${
            doInfo.timeStamp
          },"activityDate":"${
            homePage.activityDate
          }","apiMapping":"/api/task/getReward"}&t=${Date.now()}&loginType=2`
  ...
  if (answerCode) {
    if (homePage.userAnswerCode === null) {
      await takeRequest(
        cookie,
        `appid=china-joy&functionId=champion_game_prod&body={"activityDate":"${
          homePage.activityDate
        }","apiMapping":"/api/checkGuess"}&t=${Date.now()}&loginType=2`
      );

      await $.wait(1000);
      console.log(`${userName},选择队伍：${questionInfo[answerCode]}`);
      let guessAnswer = await takeRequest(
        cookie,
        `appid=china-joy&functionId=champion_game_prod&body={"activityDate":"${
          homePage.activityDate
        }","answerCode":"${answerCode}","apiMapping":"/api/guessAnswer"}&t=${Date.now()}&loginType=2`
      );
      console.log(`${userName},选择返回：${JSON.stringify(guessAnswer)}`);
      await $.wait(1000);
      homePage = await takeRequest(
        cookie,
        `appid=china-joy&functionId=champion_game_prod&body={"apiMapping":"/api/homePage"}&t=${Date.now()}&loginType=2`
      );
    }
    ...
    console.log(
      `${userName},已选择队伍：${questionInfo[homePage.userAnswerCode]}`
    );
  }
  await $.wait(1000);
  if (homePage.lotteryButtonStatus === 1) {
    console.log(`${userName},进行一次抽奖`);
    let lottery = await takeRequest(
      cookie,
      `appid=china-joy&functionId=champion_game_prod&body={"activityDate":"${
        homePage.activityDate
      }","apiMapping":"/api/lottery/lottery"}&t=${Date.now()}&loginType=2`
    );
    console.log(`${userName},抽奖结果：${JSON.stringify(lottery)}`);
  }
  let shareId = homePage.shareId;
  if (shareId) {
    let initSupport = await takeRequest(
      cookie,
      `appid=china-joy&functionId=champion_game_prod&body={"shareId":"${shareId}","apiMapping":"/api/initSupport"}&t=${Date.now()}&loginType=2`
    );
    console.log(
      `\n${userName},助力码：${shareId},已被助力：${initSupport.supportedNum}次,需要被助力：${initSupport.supportNeedNum}次，当前倍数：${initSupport.doubleNum}倍`
    );
    let need =
      Number(initSupport.supportNeedNum) - Number(initSupport.supportedNum);
    if (need > 0) {
  ...
  for (let i = 0; i < activityDateList.length; i++) {
    ...
    let homePage = await takeRequest(
      cookie,
      `appid=china-joy&functionId=champion_game_prod&body={"activityDate":${activityDate},"apiMapping":"/api/homePage"}&t=${Date.now()}&loginType=2`
    );
    await $.wait(2000);
    if (homePage.lotteryButtonStatus === 2) {
      console.log(`领取${activityDate}竞猜奖励`);
      let lotteryInfo = await takeRequest(
        cookie,
        `appid=china-joy&functionId=champion_game_prod&body={"activityDate":"${activityDate}","apiMapping":"/api/lottery"}&t=${Date.now()}&loginType=2`
      );
