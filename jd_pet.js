/*
东东萌宠 更新地址： https://raw.githubusercontent.com/flatpig/signCollection/master/jd_pet.js
更新时间：2020-11-04
...
[task_local]
#东东萌宠
15 6-18/6 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_pet.js, tag=东东萌宠, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdmc.png, enabled=true

=================================Loon===================================
[Script]
cron "15 6-18/6 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_pet.js,tag=东东萌宠

===================================Surge================================
东东萌宠 = type=cron,cronexp="15 6-18/6 * * *",wake-system=1,timeout=120,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_pet.js

====================================小火箭=============================
东东萌宠 = type=cron,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_pet.js, cronexpr="15 6-18/6 * * *", timeout=200, enable=true
...
async function energyCollect() {
  ...
  if (response.code === '0') {
    message += `【第${response.result.medalNum + 1}块勋章完成进度】${
      response.result.medalPercent
    }%，还需收集${response.result.needCollectEnergy}好感\n`;
...
async function masterHelpInit() {
  ...
  if (res.code === '0' && res.resultCode === '0') {
    if (
      res.result.masterHelpPeoples &&
      res.result.masterHelpPeoples.length >= 5
    ) {
      if (!res.result.addedBonusFlag) {
        console.log('开始领取额外奖励');
        let getHelpAddedBonusResult = await request('getHelpAddedBonus');
        console.log(
          `领取30g额外奖励结果：【${getHelpAddedBonusResult.message}】`
        );
    ...
    if (
      res.result.masterHelpPeoples &&
      res.result.masterHelpPeoples.length > 0
    ) {
...
async function slaveHelp() {
  ...
  for (let code of newShareCodes) {
    ...
    if (response.code === '0' && response.resultCode === '0') {
      if (response.result.helpStatus === 0) {
        ...
        // 您今日已无助力机会
        console.log(
          `助力好友${response.result.masterNickName}失败，您今日已无助力机会`
        );
        break;
      } else if (response.result.helpStatus === 2) {
        //该好友已满5人助力，无需您再次助力
        console.log(
          `该好友${response.result.masterNickName}已满5人助力，无需您再次助力`
        );
      } else {
  ...
  if (helpPeoples && helpPeoples.length > 0) {
    message += `【您助力的好友】${helpPeoples.substr(
      0,
      helpPeoples.length - 1
    )}\n`;
  }
}
...
async function inviteFriendsInitFun() {
  ...
  if (
    $.taskInfo.inviteFriendsInit.status == 1 &&
    $.taskInfo.inviteFriendsInit.inviteFriendsNum > 0
  ) {
    // 如果有邀请过新用户,自动领取60gg奖励
    const res = await request('getInviteFriendsReward');
    if (res.code == 0 && res.resultCode == 0) {
      console.log(
        `领取邀请新用户奖励成功,获得狗粮现有狗粮${$.taskInfo.inviteFriendsInit.reward}g，${res.result.foodAmount}g`
      );
...
function readShareCode() {
  return new Promise((resolve) => {
    $.get(
      { url: `http://api.turinglabs.net/api/v1/jd/pet/read/${randomCount}/` },
      (err, resp, data) => {
        try {
          if (err) {
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&appid=wh5&loginWQBiz=pet-town&body=${escape(
      JSON.stringify(body)
    )}`,
    headers: {
      Cookie: cookie,
