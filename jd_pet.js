...
async function jdPet() {
  try {
    ...
    if (
      ...
      await showMsg();
      console.log(
        '全部任务完成, 如果帮助到您可以点下🌟STAR鼓励我一下, 明天见~'
      );
  ...
  for (let code of newShareCodes) {
    ...
    if (response.code === '0' && response.resultCode === '0') {
      if (response.result.helpStatus === 0) {
        console.log(
          '已给好友: 【' + response.result.masterNickName + '】助力成功'
        );
...
function readShareCode() {
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `http://share.turinglabs.net/api/v3/pet/query/${randomCount}/`,
        timeout: 10000
      },
      (err, resp, data) => {
        try {
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    if ($.isNode()) {
      ...
      console.log(
        `\nBoxJs设置的${$.name}好友邀请码:${
          $.getdata('jd_pet_inviter') ? $.getdata('jd_pet_inviter') : '暂无'
        }\n`
      );
    }
