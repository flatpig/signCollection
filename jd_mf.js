...
function doInteractiveAssignment(extraType, encryptProjectId, sourceCode, encryptAssignmentId, itemId, actionType = '') {
  return new Promise(resolve => {
    $.post(
      taskUrl('doInteractiveAssignment', { encryptProjectId: encryptProjectId, encryptAssignmentId: encryptAssignmentId, sourceCode: sourceCode, itemId: itemId, actionType: actionType, completionFlag: '', ext: {} }),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
