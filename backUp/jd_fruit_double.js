...
[task_local]
#jd免费水果
5 6-18/6 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_fruit.js, tag=东东农场, img-url=https://raw.githubusercontent.com/znz1992/Gallery/master/jdsg.png, enabled=true
// Loon
[Script]
cron "5 6-18/6 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_fruit.js,tag=东东农场
// Surge
// 宠汪汪偷好友积分与狗粮 = type=cron,cronexp="5 6-18/6 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy_steal.js
  ...
  if (farmInfo.farmUserPro) {
        ...
        for (let item of clockInInit.themes) {
          if (!item.hadGot) {
            ...
            if (themeStep1.code === '0') {
              let themeStep2 = yield clockInFollowForFarm(
                item.id,
                'theme',
                '2'
              );
      ...
      if (clockInInit.venderCoupons && clockInInit.venderCoupons.length > 0) {
        for (let item of clockInInit.venderCoupons) {
          if (!item.hadGot) {
            ...
            let venderCouponStep1 = yield clockInFollowForFarm(
              item.id,
              'venderCoupon',
              '1'
            );
            ...
            if (venderCouponStep1.code === '0') {
              let venderCouponStep2 = yield clockInFollowForFarm(
                item.id,
                'venderCoupon',
                '2'
              );
    ...
    if (executeWaterRain) {
      ...
      if (executeWaterRain) {
        ...
        if (waterRain.code === '0') {
          ...
          message += `【第${farmTask.waterRainInit.winTimes + 1}次水滴雨】获得${
            waterRain.addEnergy
          }g水滴\n`;
        }
      }
    ...
    if (initForTurntableFarmRes.code === '0') {
      ...
      if (!timingGotStatus) {
        ...
        if (
          sysTime >
          timingLastSysTime + 60 * 60 * timingIntervalHours * 1000
        ) {
      ...
      if (
        ...
        if (!initForTurntableFarmRes.turntableBrowserAds[0].status) {
          const browserForTurntableFarmRes = yield browserForTurntableFarm(
            initForTurntableFarmRes.turntableBrowserAds[0].adId
          );
          if (
            browserForTurntableFarmRes.code === '0' &&
            browserForTurntableFarmRes.status
          ) {
            const browserForTurntableFarm2Res = yield browserForTurntableFarm2(
              initForTurntableFarmRes.turntableBrowserAds[0].adId
            );
      ...
      console.log(
        `---天天抽奖次数remainLotteryTimes----${remainLotteryTimes}次`
      );
      //抽奖
      if (remainLotteryTimes > 0) {
        ...
        for (
          let i = 0;
          i < new Array(remainLotteryTimes).fill('').length;
          i++
        ) {
        ...
        if (lotteryResult) {
          ...
          message += `【天天抽奖】${lotteryResult.substr(
            0,
            lotteryResult.length - 1
          )}\n`;
        }
      } else {
      ...
      for (
        ...
        if (waterResult.code === '0') {
          ...
          if (waterResult.finished) {
            ...
            if (
              waterResult.waterStatus === 0 &&
              waterResult.treeEnergy === 10
            ) {
              console.log('果树发芽了,奖励30g水滴');
              let gotStageAwardForFarmRes1 = yield gotStageAwardForFarm('1');
              console.log(
                `浇水阶段奖励1领取结果 ${JSON.stringify(
                  gotStageAwardForFarmRes1
                )}`
              );
              ...
              console.log(
                `浇水阶段奖励2领取结果 ${JSON.stringify(
                  gotStageAwardForFarmRes2
                )}`
              );
              ...
              console.log(
                `浇水阶段奖励3领取结果 ${JSON.stringify(
                  gotStageAwardForFarmRes3
                )}`
              );
    ...
    if (
      ...
      for (
        ...
        console.log(
          `本次浇水结果(水果马上就可兑换了):   ${JSON.stringify(resp)}`
        );
        if (resp.code === '0') {
          console.log('\n浇水10g成功\n');
          if (resp.finished) {
            ...
            console.log(
              `目前水滴【${resp.totalEnergy}】g,继续浇水，水果马上就可以兑换了`
            );
          }
        } else {
          console.log('浇水出现失败异常,跳出不在继续浇水');
    ...
    farmInfo = yield initForFarm();
    message += `【水果🍉进度】${(
      (farmInfo.farmUserPro.treeEnergy / farmInfo.farmUserPro.treeTotalEnergy) *
      100
