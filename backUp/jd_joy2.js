...
// [task_local]
// #京东宠汪汪
// 15 1,2 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy.js, tag=京东宠汪汪, img-url=https://raw.githubusercontent.com/znz1992/Gallery/master/jdww.png, enabled=true
// Loon
// [Script]
// cron "15 1,2 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy.js,tag=京东宠汪汪
const name = '京东宠汪汪';
const $ = new Env(name);
...
//直接用NobyDa的jd cookie
let cookie = jdCookieNode.CookieJD
...
function* step() {
  ...
  if (cookie) {
    ...
    if (petTaskConfig.success) {
      //每日签到
      let signEveryDayTask = petTaskConfig.datas.find(
        (item) => item.taskType === 'SignEveryDay'
      );
      if (
        signEveryDayTask &&
        signEveryDayTask.taskStatus == 'processing' &&
        signEveryDayTask.joinedCount == 0
      ) {
      ...
      //关注店铺
      let followShopTask = petTaskConfig.datas.find(
        (item) => item.taskType === 'FollowShop'
      );
      if (
        followShopTask &&
        followShopTask.taskStatus == 'processing' &&
        followShopTask.taskChance > followShopTask.joinedCount
      ) {
        for (let shop of followShopTask.followShops) {
          if (!shop.status) {
            let followShopResult = yield followShop(shop.shopId);
            console.log(
              `关注店铺${shop.name}结果${JSON.stringify(followShopResult)}`
            );
          }
        }
      } else {
      ...
      //三餐
      let threeMeals = petTaskConfig.datas.find(
        (item) => item.taskType === 'ThreeMeals'
      );
      ...
      //逛会场
      let scanMarketTask = petTaskConfig.datas.find(
        (item) => item.taskType === 'ScanMarket'
      );
      if (
        scanMarketTask &&
        scanMarketTask.taskStatus == 'processing' &&
        scanMarketTask.taskChance > scanMarketTask.joinedCount
      ) {
        for (let market of scanMarketTask.scanMarketList) {
          if (!market.status) {
            ...
            console.log(
              `逛会场点击${market.marketName}结果${JSON.stringify(clickResult)}`
            );

            let scanMarketResult = yield ScanMarket(market.marketLinkH5);
            console.log(
              `逛会场${market.marketName}结果${JSON.stringify(
                scanMarketResult
              )}`
            );
          }
      ...
      //关注商品
      let followGoodTask = petTaskConfig.datas.find(
        (item) => item.taskType === 'FollowGood'
      );
      if (
        followGoodTask &&
        followGoodTask.taskStatus == 'processing' &&
        followGoodTask.taskChance > followGoodTask.joinedCount
      ) {
        for (let good of followGoodTask.followGoodList) {
          if (!good.status) {
            let followGoodResult = yield followGood(good.sku);
            console.log(
              `关注商品${good.skuName}结果${JSON.stringify(followGoodResult)}`
            );
          }
        }
      } else {
      ...
      //浏览频道
      let followChannelTask = petTaskConfig.datas.find(
        (item) => item.taskType === 'FollowChannel'
      );
      if (
        followChannelTask &&
        followChannelTask.taskStatus == 'processing' &&
        followChannelTask.taskChance > followChannelTask.joinedCount
      ) {
        for (let channel of followChannelTask.followChannelList) {
          if (!channel.status) {
            let followChannelResult = yield FollowChannel(channel.channelId);
            console.log(
              `浏览频道${channel.channelName}结果${JSON.stringify(
                followChannelResult
              )}`
            );
          }
      ...
      if (deskGoodDetails.success) {
        if (
          deskGoodDetails.data.deskGoods &&
          deskGoodDetails.data.deskGoods.length > 0
        ) {
          for (let deskGood of deskGoodDetails.data.deskGoods) {
            if (!deskGood.status) {
              let scanDeskGoodResult = yield ScanDeskGood(deskGood.sku);
              console.log(
                `浏览频道${deskGood.skuName}结果${JSON.stringify(
                  scanDeskGoodResult
                )}`
              );
            }
        ...
        console.log(
          `浏览商品奖励积分返回结果${JSON.stringify(deskGoodDetails)}`
        );
      }
      // 看激励视频得狗粮
      ...
      if (taskVideoRes.success) {
        ...
        if (taskArr.taskChance === joinedCount) {
          ...
          for (
            let i = 0;
            i < new Array(taskArr.taskChance - joinedCount).fill('').length;
            i++
          ) {
      ...
      if (getFriendsResult.page && getFriendsResult.datas) {
        ...
        for (var i = getFriendsResult.datas.length - 1; i >= 1; i--) {
          ...
          if (enterFriendRoomResult.data['friendHomeCoin'] > 0) {
            ...
            console.log(
              `收取好友金币结果${JSON.stringify(getFriendCoinResult)}`
            );
          }
          ...
          if (getFriendsResult.datas[i]['stealStatus'] == 'can_steal') {
            ...
            console.log(
              `收取好友狗粮结果${JSON.stringify(getRandomFoodResult)}`
            );
          }
...
function click(marketLink) {
  request(
    `https://jdjoy.jd.com/pet/icon/click?reqSource=h5&iconCode=scan_market&linkAddr=${marketLink}`
  );
}
...
function ScanDeskGood(sku) {
  requestPost(
    `https://jdjoy.jd.com/pet/scan`,
    JSON.stringify({ sku: sku, taskType: 'ScanDeskGood', reqSource: 'h5' }),
    'application/json'
  );
}
...
function getFriends(currentPage) {
  request(
    `https://jdjoy.jd.com/pet/getFriends?itemsPerPage=20&currentPage=${currentPage}`
  );
}
...
function sanVideo() {
  ...
  const option = {
    ...
    headers: {
      ...
      'User-Agent':
        'Mozilla/5.0 (iPhone; CPU iPhone OS 13_5_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/7.0.14(0x17000e2b) NetType/4G Language/zh_CN',
      Referer:
        'https://servicewechat.com/wxccb5c536b0ecd1bf/617/page-frame.html',
    },
  };
...
function requestPost(url, body, ContentType) {
  ...
  const options = {
    ...
    headers: {
      ...
      Host: 'jdjoy.jd.com',
      Referer:
        'https://jdjoy.jd.com/pet/index?un_area=5_274_49707_49973&lng=116.8439659502069&lat=39.95722551778479',
      'Accept-Language': 'zh-cn',
      'Accept-Encoding': 'gzip, deflate, br',
    },
  };
