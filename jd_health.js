...
let cookiesArr = [],
  ...
  message;
const inviteCodes = [];
let reward = process.env.JD_HEALTH_REWARD_NAME
  ? process.env.JD_HEALTH_REWARD_NAME
  : '';
...
if ($.isNode()) {
  ...
  console.log(
    `如果出现提示 ?.data. 错误，请升级nodejs版本(进入容器后，apk add nodejs-current)`
  );
...
function getTaskDetail(taskId = '') {
  return new Promise((resolve) => {
    $.get(
      taskUrl('jdhealth_getTaskDetail', {
        buildingId: '',
      ...
      async (err, resp, data) => {
        try {
          if (safeGet(data)) {
            ...
            if (taskId === -1) {
              let tmp = parseInt(
                parseFloat(data?.data?.result?.userScore ?? '0')
              );
              if (!$.earn) {
                $.score = tmp;
                $.earn = 1;
              ...
              console.log(
                `${data?.data?.result?.taskVos[0]?.taskName}任务，完成次数：${data?.data?.result?.taskVos[0]?.times}/${data?.data?.result?.taskVos[0]?.maxTimes}`
              );
              if (
                data?.data?.result?.taskVos[0]?.times ===
                data?.data?.result?.taskVos[0]?.maxTimes
              )
                return;
              await doTask(
                data?.data?.result?.taskVos[0].shoppingActivityVos[0]
                  ?.taskToken,
                22,
                1
              ); //领取任务
              await $.wait(
                1000 * (data?.data?.result?.taskVos[0]?.waitDuration || 3)
              );
              await doTask(
                data?.data?.result?.taskVos[0].shoppingActivityVos[0]
                  ?.taskToken,
              ...
              for (let vo of data?.data?.result?.taskVos.filter(
                (vo) => vo.taskType !== 19
              ) ?? []) {
                console.log(
                ...
                for (let i = vo.times; i < vo.maxTimes; i++) {
                  ...
                  if (vo.taskType === 13) {
                    ...
                    await doTask(
                      vo.shoppingActivityVos[0]?.taskToken,
                      vo?.taskId,
                      1
                    );
                    await $.wait(1000 * 10);
          ...
          if (data.data.bizCode === 0 || data.data.bizMsg === 'success') {
            ...
            if ($.isNode()) {
              allMessage += `【京东账号${$.index}】 ${$.UserName}\n兑换${
                data.data.result.jingBeanNum
              }京豆成功🎉${$.index !== cookiesArr.length ? '\n\n' : ''}`;
            }
          }
        }
      } catch (e) {
        ...
        if (safeGet(data)) {
          ...
          if ([0, 1].includes(data?.data?.bizCode ?? -1)) {
            ...
            if (data?.data?.result?.score)
              console.log(
                `任务完成成功，获得：${data?.data?.result?.score ?? '未知'}能量`
              );
            else
              console.log(
            ...
            console.log(
              `任务完成失败：${data?.data?.bizMsg ?? JSON.stringify(data)}`
            );
          }
        }
      } catch (e) {
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&body=${escape(
      JSON.stringify(body)
    )}&client=wh5&clientVersion=1.0.0&uuid=`,
    headers: {
      Cookie: cookie,
