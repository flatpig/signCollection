...
!(async () => {
  ...
  let res = await getAuthorShareCode(
    'https://raw.githubusercontent.com/Aaron-lv/updateTeam/master/shareCodes/cfd.json'
  );
  if (!res) {
    $.http
      .get({
        url: 'https://purge.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/cfd.json'
      })
      .then((resp) => {})
      .catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000);
    res = await getAuthorShareCode(
      'https://cdn.jsdelivr.net/gh/Aaron-lv/updateTeam@master/shareCodes/cfd.json'
    );
  }
...
async function composeGameState(type = true) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(`user/ComposeGameState`, `dwFirst=1`),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} ComposeGameState API请求失败，请检查网路重试`
            );
          } else {
            ...
            if (type) {
              ...
              if (data.iRet === 0) {
                if (
                  data.dwCurProgress <
                    data.stagelist[data.stagelist.length - 1]
                      .dwCurStageEndCnt &&
                  data.strDT
                ) {
                  let count =
                    data.stagelist[data.stagelist.length - 1].dwCurStageEndCnt;
                  console.log(
                    `当前已合成${data.dwCurProgress}颗珍珠，还需合成珍珠${
                  ...
                  for (let key of Object.keys(composeGameStateRes.stagelist)) {
                    ...
                    if (
                      vo.dwIsAward == 0 &&
                      composeGameStateRes.dwCurProgress >= vo.dwCurStageEndCnt
                    ) {
                      await $.wait(2000);
...
function composeGameAddProcess(strDT) {
  return new Promise((resolve) => {
    $.get(
      taskUrl(`user/ComposeGameAddProcess`, `strBT=${strDT}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} ComposeGameAddProcess API请求失败，请检查网路重试`
            );
          } else {
            data = JSON.parse(data);
          }
...
function composeGameAward(dwCurStageEndCnt) {
  return new Promise((resolve) => {
    $.get(
      taskUrl(`user/ComposeGameAward`, `dwCurStageEndCnt=${dwCurStageEndCnt}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            console.log(
              `${$.name} ComposeGameAward API请求失败，请检查网路重试`
            );
          } else {
            data = JSON.parse(data);
            if (data.iRet === 0) {
    ...
    $.get(taskUrl(`user/EmployTourGuideInfo`), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`);
          console.log(
            `${$.name} EmployTourGuideInfo API请求失败，请检查网路重试`
          );
        } else {
          data = JSON.parse(data);
          console.log(`雇导游`);
          ...
          for (let key of Object.keys(data.TourGuideList)) {
            ...
            if (vo.ddwRemainTm === 0 && vo.ddwProductCoin !== minProductCoin) {
              ...
              console.log(
                `【${buildNmae}】雇佣费用：${vo.ddwCostCoin}金币 增加收益：${vo.ddwProductCoin}金币`
              );
...
function getUserInfo(showInvite = true) {
  return new Promise(async (resolve) => {
    $.get(
      taskUrl(
        `user/QueryUserInfo`,
        `ddwTaskId=&strShareId=&strMarkList=${escape(
...
function doTask(taskId, type = 1) {
  return new Promise(async (resolve) => {
    switch (type) {
      ...
      case 2:
        $.get(
          taskListUrl(`DoTask`, `taskId=${taskId}`, `jxbfddch`),
          (err, resp, data) => {
            try {
              if (err) {
                console.log(`${JSON.stringify(err)}`);
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    if ($.isNode()) {
      ...
      if ($.getdata('jd_jxCFD'))
        $.shareCodesArr = $.getdata('jd_jxCFD')
          .split('\n')
          .filter((item) => !!item);
