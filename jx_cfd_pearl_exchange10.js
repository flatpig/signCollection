...
/*
0 0-23/5 * * * jx_cfd_pearl_exchange10.js
* */
// noinspection JSUnresolvedFunction
const { Env } = require('./magic');
const $ = new Env('M财富岛珍珠兑换10');
let money = 10;
$.logic = async function () {
  ...
  for (let i = 0; i < prizeInfos.length; i++) {
    ...
    if (money === number) {
      ...
      for (let j = 0; j < 3; j++) {
        if (await ExchangePearlHb(prizeInfo.dwLvl, prizeInfo.ddwVirHb, prizeInfo.strPool)) {
          break;
        }
...
$.run({ filename: __filename, wait: [3000, 5000] }).catch(reason => console.log(reason));
