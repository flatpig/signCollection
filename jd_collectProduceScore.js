...
每小时的第20分运行一次
20 * * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_collectProduceScore.js
 */
...
function stall_collectProduceScore(body) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('stall_collectProduceScore', body),
      (err, resp, data) => {
        try {
          if (err) {
            ...
            if (data) {
              ...
              if (data && data.data.bizCode === 0) {
                console.log(
                  `京东账号${$.index} ${$.UserName}成功收集金币:${data.data.result.produceScore}个`
                );
              } else {
                console.log(
                  `京东账号${$.index} ${$.UserName}成功收集金币失败:${data.data.bizMsg}`
                );
              }
            } else {
...
function taskPostUrl(functionId, body = {}) {
  return {
    ...
    headers: {
      'User-Agent':
        'jdapp;iPhone;9.2.0;14.1;;network/wifi;Mozilla/5.0 (iPhone; CPU iPhone OS 14_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1',
      'Content-Type': 'application/x-www-form-urlencoded',
      Host: 'api.m.jd.com',
      Cookie: cookie,
      Origin: 'https://wbbny.m.jd.com',
      Referer:
        'https://wbbny.m.jd.com/babelDiy/Zeus/4SJUHwGdUQYgg94PFzjZZbGZRjDd/index.html',
    },
  };
}
function Env(t, s) {
