...
$.newShareCodes = [
  // '21d9b4b51a69839577027beb0aad5105',
  // '8edbdfa148e78f028496cff17e7df35b'
];
!(async () => {
  if (!cookiesArr[0]) {
    $.msg(
...
function queryShareInfo() {
  return new Promise((resolve) => {
    $.get(
      taskUrl('jdh_invite_startInvite', {
        channel: 'jdhapp',
        m_patch_appid: 'jdh'
      }),
      async (err, resp, data) => {
...
function queryInviteHome() {
  ...
  return new Promise((resolve) => {
    $.get(
      taskUrl('jdh_invite_queryInviteHome', {
        channel: 'jdhapp',
        m_patch_appid: 'jdh'
      }),
      async (err, resp, data) => {
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.data.ownerInfo.firstVisitChance) {
                console.log(
                  `首次访问成功，获得 ${data.data.ownerInfo.firstVisitChance}张汪汪卡`
                );
              }
            }
          }
...
function helpFriend(code) {
  ...
  return new Promise((resolve) => {
    $.get(
      taskUrl('jdh_invite_inviteFriends', body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                ...
                console.log(`助力好友 ${code} 失败，错误信息：${data.message}`);
            }
          }
        } catch (e) {
          $.logErr(e, resp);
...
function getTaskList() {
  let body = {
    pageSize: 15,
    startFloor: 1,
    pageId: 'c7c1fa16b8a94fbb97f6ec220488d01b'
  };
  return new Promise((resolve) => {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            // console.log(data)
            $.inviteInfo = data.data.floorDataList.filter(
              (vo) => vo.name === 'HD_Floor_Health_Month_CollectCard'
            )[0];
            console.log($.inviteInfo);
            console.log(
              `当前助力进度：${$.inviteInfo.items[0].completeNum}/${$.inviteInfo.items[0].limitNum}`
            );
          }
        }
      } catch (e) {
...
async function showMsg() {
  message = `获得${$.count - $.nowCount}张汪汪卡，共${
    $.count
  }张汪汪卡\n任务已做完，请手动领取奖励`;
  if ($.isNode() && !jdNotify) {
    await notify.sendNotify(
    ...
    headers: {
      ...
      'User-Agent': $.isNode()
        ? process.env.JD_USER_AGENT
          ? process.env.JD_USER_AGENT
          : 'jdhapp;iPhone;9.2.7;14.2;network/wifi;lang/zh_CN;model/iPhone10,2;appBuild/1206;pv/2.1;apprpd/;usc/;jdv/0|;umd/;psq/4;ucp/;app_device/IOS;utr/;ref/;adk/;ads/;Mozilla/5.0 (iPhone; CPU iPhone OS 14_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1'
        : $.getdata('JDUA')
        ? $.getdata('JDUA')
        : 'jdhapp;iPhone;9.2.7;14.2;network/wifi;lang/zh_CN;model/iPhone10,2;appBuild/1206;pv/2.1;apprpd/;usc/;jdv/0|;umd/;psq/4;ucp/;app_device/IOS;utr/;ref/;adk/;ads/;Mozilla/5.0 (iPhone; CPU iPhone OS 14_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1'
    }
...
function taskUrl(function_id, body = {}) {
  return {
    url: `${JD_API_HOST}?functionId=${function_id}&appid=JDHAPP&clientVersion=2.1.7&body=${escape(
      JSON.stringify(body)
    )}`,
    headers: {
      Cookie: cookie,
