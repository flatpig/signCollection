...
function sign() {
  return new Promise(async (resolve) => {
    $.get(
      taskurl('SignIn', `date=${new Date().Format('yyyyMMdd')}&type=0`),
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data['ret'] === 0) {
                console.log(
                  `签到钞票：收取成功，获得 ${data['data']['rewardMoneyToday']}`
                );
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (
              data['ret'] === 0 &&
              data['data']['flopFinishNumber'] < data['data']['flopNumber']
            ) {
...
function selectCard(cardInfo) {
  return new Promise(async (resolve) => {
    $.get(
      taskurl(
        'SelectCard',
        `cardInfo=${JSON.stringify({ cardInfo: cardInfo })}`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
...
function finishCard(cardId) {
  return new Promise(async (resolve) => {
    $.get(
      taskurl('FinishCard', `cardid=${cardId}`),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              data = JSON.parse(data);
              if (data['ret'] === 0) {
                let ratio = data['data']['cardInfo'].filter(
                  (vo) => vo['cardId'] === cardId
                )[0]['cardRatio'];
                console.log(
                  `翻倍成功，获得${ratio}%，共计获得${data['data']['earnRatio']}%`
                );
              }
            }
          }
...
function upgrade() {
  return new Promise(async (resolve) => {
    $.get(
      taskurl(
        'UpgradeUserLevelDraw',
        `date=${new Date().Format('yyyyMMdd')}&type=0`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              console.log(
                `点击厂长成功，获得 ${data['data']['moneyNum']} 钞票`
              );
...
function assistFriend(shareId) {
  return new Promise(async (resolve) => {
    $.get(
      taskurl('AssistFriend', `shareId=${escape(shareId)}`),
      (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function readShareCode() {
  ...
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `http://api.turinglabs.net/api/v1/jd/jxstory/read/${randomCount}/`
      },
      (err, resp, data) => {
        try {
          if (err) {
...
function taskurl(functionId, body = '') {
  return {
    ...
    headers: {
      ...
      'Accept-Language': 'zh-cn',
      Referer:
        'https://st.jingxi.com/pingou/jx_factory_story/index.html?ptag=138963.4.3',
      'Accept-Encoding': 'gzip, deflate, br'
    }
  };
}
