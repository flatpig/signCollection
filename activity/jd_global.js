...
!(function (n) {
  ...
  function l(n, r) {
    var t,
      ...
      c = [];
    for (
      o[15] = c[15] = void 0, e.length > 16 && (e = i(e, 8 * n.length)), t = 0;
      t < 16;
      t += 1
    )
...
async function getHome(info = false) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('mainInfo', { activityCode: actCode }),
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data['code'] === '0') {
                ...
                if (info) {
                  ...
                  let station = activityStations.filter(
                    (vo) => vo.unlock === true
                  );
                  ...
                  console.log(
                    `当前活动：第${activityCalendar.currDays}/${activityCalendar.totalDays}天`
                  );
                }
...
function taskUrl(function_id, body = {}) {
  function getSign(data) {
    ...
    return {
      sealsTs: t,
      seals: $.md5(
        `${data.taskId}${data.inviterPin ? data.inviterPin : ''}${t}Ea6YXT`
      )
    };
  }
  if (body['taskId']) {
