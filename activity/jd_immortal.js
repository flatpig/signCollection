...
let scoreToBeans = $.isNode()
  ? process.env.JD_IMMORTAL_SCORE || 0
  : $.getdata('scoreToBeans') || 0; //兑换多少数量的京豆（20或者1000），0表示不兑换，默认兑换20京豆，如需兑换把0改成20或者1000，或者'商品名称'(商品名称放到单引号内)即可
...
function getExchangeInfo() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('mcxhd_brandcity_exchangePage'),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function getTaskList(body = {}) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('mcxhd_brandcity_taskList', body, 'mcxhd_brandcity_taskList'),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
              ...
              if (data.retCode === '200') {
                ...
                for (let vo of $.tasks) {
                  if (
                    ...
                    for (
                      let i = vo.times, j = 0;
                      i < vo.maxTimes && j < vo.subItem.length;
                      ++i, ++j
                    ) {
                      console.log(
                        `去做${vo.taskName}任务，${i + 1}/${vo.maxTimes}`
                      );
                      let item = vo['subItem'][j];
                      await doTask(item['itemToken']);
                      await $.wait(
                        (vo.waitDuration ? vo.waitDuration : 5 + 1) * 1000
                      );
                    }
...
function requireConfig() {
  return new Promise(async (resolve) => {
    ...
    if ($.isNode()) {
      ...
      $.cor = process.env.JD_IMMORTAL_LATLON
        ? JSON.parse(process.env.JD_IMMORTAL_LATLON)
        : await getLatLng();
    } else {
      $.cor = $.getdata('IMMORTAL_LATLON')
        ? JSON.parse($.getdata('IMMORTAL_LATLON'))
        : {};
    }
...
function taskPostUrl(function_id, body = {}, function_id2) {
  ...
  return {
    ...
    body: `functionId=${function_id}&body=${escape(
      JSON.stringify(body)
    )}&client=wh5&clientVersion=1.0.0&appid=publicUseApi`,
    headers: {
      Cookie: cookie,
