...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let k = 0; k < $.inviteCodeList.length; k++) {
      ...
      console.log(
        `${$.UserName}去助力：${$.uniqueId} 活动，助力码：${$.oneCode}`
      );
  ...
  for (let i = 0; i < $.rewards.length; i++) {
    if ($.rewards[i].prizeType === 1) {
      ...
      if (!$.rewards[i].fillReceiverFlag) {
        ...
        sendMessage += `【京东账号${$.index}】${$.UserName}，获得实物：${
          $.rewards[i].prizeDesc || ''
        }\n`;
      } else {
...
async function guafen() {
  ...
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${
    $.shopId
  }%22,%22nowTime%22:%22${a}%22,%22token%22:%22${
    $.tokenId
...
async function help() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${
    $.shopId
  }%22,%22itemToken%22:%22${$.helpCode}%22,%22token%22:%22${
    $.tokenId
...
async function drawAward() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${
    ...
    $.tokenId
  }%22%7D&appid=xdz&functionId=mcxhd_starmall_drawAward&t=${Date.now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    Origin: `https://h5.m.jd.com`,
    Cookie: $.cookie,
...
async function doXdzTask() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${
    $.shopId
  }%22,%22itemToken%22:%22${$.subItemInfo.itemToken}%22,%22token%22:%22${
    $.tokenId
  }%22%7D&appid=xdz&functionId=mcxhd_starmall_doTask&t=${Date.now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
...
async function getXdzUseInfo() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${
    ...
    $.tokenId
  }%22%7D&appid=xdz&functionId=mcxhd_starmall_taskList&t=${Date.now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    Origin: `https://h5.m.jd.com`,
    Cookie: $.cookie,
...
async function getXdzInfo() {
  const url = `https://api.m.jd.com/?body=%7B%22shopId%22:%22${
    ...
    $.tokenId
  }%22%7D&appid=xdz&functionId=mcxhd_starmall_getStarShopPage&t=${Date.now()}&loginWQBiz=`;
  const method = `GET`;
  const headers = {
    Origin: `https://h5.m.jd.com`,
    Cookie: $.cookie,
...
async function starShop() {
  ...
  for (let i = 0; i < $.taskList.length; i++) {
    ...
    if ($.oneTask.taskType === 'SIGN') {
      ...
      $.oneTask.taskType === 'BROWSE_CHANNEL' ||
      $.oneTask.taskType === 'FOLLOW_SHOP'
    ) {
      $.taskDetail = {};
      $.taskItemList = [];
  ...
  switch (type) {
    ...
    case 'apDoTask':
      body = `functionId=apDoTask&body={"taskType":"${
        ...
        $.uniqueId
      }","channel":4,"linkId":"Y2aqxng42hZ0eGxGtbCMiQ","itemId":"${encodeURIComponent(
        $.oneItemInfo.itemId
