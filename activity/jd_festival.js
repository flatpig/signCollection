...
async function festival() {
  ...
  await getSignInfo();
  await getTask(true);
  for (let i = 0; i < 4; ++i) await getTask();
  for (let i = 0; i < 2; ++i) await getExtTask();
  await helpFriends();
}
      ...
      try {
        if (err) {
          ...
          if (safeGet(data)) {
            data = JSON.parse(data);
            if (data.code === 200) {
              let vo = data.data.signDayList.filter(
                (vo) => vo.date === data.data.today
              )[0];
              if (vo.flag === '1') console.log(`今日已签到`);
              else await signIn();
            } else {
              console.log(`签到信息获取失败！`);
            }
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.code === 200) {
              ...
              if (share)
                console.log(
                  `您的好友助力码为：${shareTask.shareId}，当前助力进度：${
                    shareTask.needSupportNum - shareTask.remainSupportNum
                  }/${shareTask.needSupportNum}`
                );
              else
...
function doReceive(type) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('task/doReceive', { type: type }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function browseTask(id, type) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('extTask/browse', { id: id, type: type }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function getPrize(id, type) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('extTask/getPrize', { id: id, type: type }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function addMoney(sum) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('index/addGoldNums', { goldNum: sum }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function doSupport(shareId) {
  return new Promise((resolve) => {
    $.get(
      taskUrl('task/doSupport', { shareId: shareId }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function taskPostUrl(function_id, body = {}) {
  ...
  return {
    ...
    headers: {
      ...
      sign: sign(
        n,
        `d55b480bed0545839dbd8b78b6cffdb1${t}`,
        `/sf/${function_id}`
      ),
