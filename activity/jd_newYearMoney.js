...
!(async () => {
  ...
  if (receiveAccount.length) console.log(`开始领卡`);
...
function showMsg() {
  return new Promise((resolve) => {
    if (!$.risk)
      message += `本次运行获得${Math.round($.red * 100) / 100}红包，共计红包${
        $.total
      }`;
    if (!jdNotify) {
      $.msg($.name, '', `${message}`);
      ...
      try {
        if (err) {
          ...
          if (data && data.data['bizCode'] === 0) {
            ...
            await $.wait(2000);
            for (let i = 1; i <= 6; ++i) {
              let cards = data.data.result.cardInfos.filter(
                (vo) => vo.cardType === i
              );
...
function lotteryHundredCard() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('newyearmoney_lotteryHundredCard'),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function showHundredCardInfo(cardNo) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('newyearmoney_showHundredCardInfo', { cardNo: cardNo }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function receiveHundredCard(cardNo) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('newyearmoney_receiveHundredCard', { cardNo: cardNo }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function consumeCard(cardNo) {
  return new Promise((resolve) => {
    setTimeout(() => {
      $.post(
        taskPostUrl('newyearmoney_consumeCard', { cardNo: cardNo }),
        async (err, resp, data) => {
          try {
            if (err) {
              console.log(`${JSON.stringify(err)}`);
              ...
              if (data && data.data['bizCode'] === 0) {
                ...
                console.log(
                  `合成成功，获得${data.data.result.currentTimeMoney}红包`
                );
              } else {
                $.risk = true;
                console.log(`账号被风控，无法参与活动`);
...
function helpFriend(inviteId) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('newyearmoney_assist', { inviteId: inviteId }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function sendCard(cardNo) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('newyearmoney_sendCard', { cardNo: cardNo }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function receiveCard(token) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('newyearmoney_receiveCard', { token: token }),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
