...
function jdsplit_getLottery(taskId) {
  return new Promise((resolve) => {
    ...
    $.post(
      taskPostUrl('splitHongbao_getLotteryResult', body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                console.log(
                  `红包领取结果：${data.data.result.userAwardsCacheDto.redPacketVO.name}`
                );
              } else {
...
function jdsplit_getTaskDetail() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('splitHongbao_getHomeData', {
        appId: '1EFRTwA',
        taskToken: ''
      }),
      async (err, resp, data) => {
