          ...
          if (safeGet(data)) {
            ...
            for (let i = 0; i < vo.venueList.length; ++i) {
              ...
              if (venue.venueStatus === 1) {
                console.log(`【浏览会场】`);
                await doTask(
                  `starId=PUBG&type=venue&id=${venue.venueId}&status=1`
                );
                await $.wait(10000);
                await doTask(
                  `starId=PUBG&type=venue&id=${venue.venueId}&status=2`
                );
              }
            }
            for (let i = 0; i < vo.productList.length; ++i) {
              ...
              if (product.productStatus === 1) {
                ...
                await doTask(
                  `starId=PUBG&type=product&id=${product.productId}&status=1`
                );
                await $.wait(10000);
                await doTask(
                  `starId=PUBG&type=product&id=${product.productId}&status=2`
                );
              }
              if (product.productStatus === 2) {
                console.log(
                  `【浏览商品】浏览商品 ${product.productName}未领奖，去领奖`
                );
                await doTask(
                  `starId=PUBG&type=product&id=${product.productId}&status=2`
                );
            ...
            for (let i = 0; i < vo.shopList.length; ++i) {
              ...
              if (shop.shopStatus === 0 || shop.shopStatus === 1) {
                ...
                await doTask(
                  `starId=PUBG&type=shop&id=${shop.shopId}&status=1`
                );
                await $.wait(10000);
                await doTask(
                  `starId=PUBG&type=shop&id=${shop.shopId}&status=2`
                );
              }
              if (shop.shopStatus === 2) {
                console.log(
                  `【关注店铺】关注店铺 ${shop.shopName}未领奖，去领奖`
                );
                await doTask(
                  `starId=PUBG&type=shop&id=${shop.shopId}&status=2`
                );
