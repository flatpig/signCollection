...
async function jdHealth() {
  ...
  if ($.userInfo.scorePerLottery <= $.userInfo.userScore) {
    console.log(
      `当前用户金币: ${$.userInfo.userScore}，抽奖需要${$.userInfo.scorePerLottery}，开始抽奖`
    );
    message += `当前用户金币: ${$.userInfo.userScore}，抽奖需要${$.userInfo.scorePerLottery}，开始抽奖\n`;
    for (
      let i = 0;
      i < parseInt($.userInfo.userScore) / $.userInfo.scorePerLottery;
      ++i
    ) {
    ...
    console.log(
      `当前用户金币: ${$.userInfo.userScore}，抽奖需要${$.userInfo.scorePerLottery}，金币不足无法抽奖`
    );
...
function jdhealth_collectScore(taskToken, taskId, itemId, actionType = 0) {
  return new Promise((resolve) => {
    ...
    $.post(
      taskPostUrl('harmony_collectScore', body),
      async (err, resp, data) => {
        try {
          if (err) {
...
function jdhealth_getLottery() {
  return new Promise((resolve) => {
    ...
    $.post(
      taskPostUrl('interact_template_getLotteryResult', body),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                console.log(
                  `抽奖成功，获得：${JSON.stringify(
                    data.data.result.userAwardsCacheDto
                  )}`
                );
                message += `抽奖成功，获得：${JSON.stringify(
                  data.data.result.userAwardsCacheDto
                )}\n`;
              } else {
...
function jdhealth_getTaskDetail(get = 1) {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('healthyDay_getHomeData', {
        appId: '1EFRTwg',
        taskToken: ''
      }),
      async (err, resp, data) => {
...
function readShareCode() {
  ...
  return new Promise(async (resolve) => {
    $.get(
      {
        url: `http://api.turinglabs.net/api/v1/jd/jdhealth/read/${randomCount}/`
      },
      (err, resp, data) => {
        try {
          if (err) {
