      ...
      try {
        if (err) {
          ...
          if (data && data.data['bizCode'] === 0) {
            ...
            console.log(
              `当前爆竹${$.userInfo.raiseInfo.remainScore}🧨，下一关需要${
...
function pkCollectScore() {
  return new Promise((resolve) => {
    $.post(
      taskPostUrl('nian_pk_collectScore', {}, 'nian_pk_collectScore'),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function arStart(level) {
  return new Promise((resolve) => {
    $.post(
      taskArPostUrl('arNianStartGame.do', `level=${level}`),
      async (err, resp, data) => {
        try {
          if (err) {
            console.log(`${JSON.stringify(err)}`);
...
function arEnd(level, random) {
  return new Promise((resolve) => {
    $.post(
      taskArPostUrl(
        'arNianEndGame.do',
        `random=${random}&type=1&level=${level}`
      ),
      async (err, resp, data) => {
        try {
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.code === 0) {
                ...
                console.log(
                  `游戏完成成功，获得${
                    data.rv.winAward + data.rv.firstWin
                  }🧨，通关情况：${level}/${maxLevel}`
                );
              } else {
                console.log(`游戏开启失败，错误：${JSON.stringify(data)}`);
...
function taskArGetUrl(function_id) {
  ...
  return {
    ...
    headers: {
      ...
      referer:
        'https://h5.m.jd.com/babelDiy/Zeus/2ZUbtdUfe8ZTyCQrVecyjdNehHpL/index.html'
    }
  };
}
