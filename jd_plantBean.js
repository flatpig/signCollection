...
async function jdPlantBean() {
  try {
    ...
    if (
      ...
      message += `【上期时间】${roundList[num - 1].dateDesc.replace(
        '上期 ',
        ''
      )}\n`;
...
async function stealFriendWater() {
  ...
  if ($.stealFriendList && $.stealFriendList.code === '0') {
    ...
    console.log(
      `$.stealFriendList 异常： ${JSON.stringify($.stealFriendList)}`
    );
  }
}
async function doEgg() {
  ...
  if ($.plantEggLotteryRes && $.plantEggLotteryRes.code === '0') {
    ...
    console.log(
      '查询天天扭蛋的机会失败' + JSON.stringify($.plantEggLotteryRes)
    );
  }
}
