/*
种豆得豆 脚本更新地址：https://raw.githubusercontent.com/flatpig/signCollection/master/jd_plantBean.js
更新时间：2020-11-04
...
======================================Surge==========================
京东种豆得豆 = type=cron,cronexp="1 7-21/2 * * *",wake-system=1,timeout=120,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_plantBean.js

====================================小火箭=============================
京东种豆得豆 = type=cron,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_plantBean.js, cronexpr="1 7-21/2 * * *", timeout=200, enable=true
...
async function doGetReward() {
  console.log(
    `【上轮京豆】${
      awardState === '4' ? '采摘中' : awardState === '5' ? '可收获了' : '已领取'
    }`
  );
  if (awardState === '4') {
    //京豆采摘中...
...
async function stealFriendWater() {
  ...
  if ($.stealFriendList.code === '0') {
    ...
    if (
      $.stealFriendList.data &&
      $.stealFriendList.data.friendInfoList &&
  ...
  if ($.plantEggLotteryRes.code === '0') {
    if ($.plantEggLotteryRes.data.restLotteryNum > 0) {
      ...
      for (let i = 0; i < eggL.length; i++) {
        ...
        console.log(
          `天天扭蛋成功：${JSON.stringify($.plantEggDoLotteryResult)}`
        );
      }
    } else {
      console.log('暂无扭蛋机会');
  ...
  if ($.taskList && $.taskList.length > 0) {
    for (let item of $.taskList) {
      ...
      if (item.dailyTimes === 1 && item.taskType !== 8) {
        ...
        console.log(
          `做 ${item.taskName}任务结果:${JSON.stringify(
            $.receiveNutrientsTaskRes
          )}\n`
        );
      }
      if (item.taskType === 3) {
...
async function doHelp() {
  for (let plantUuid of newShareCodes) {
    console.log(
      `开始助力京东账号${$.index} - ${$.nickName}的好友: ${plantUuid}`
    );
...
function readShareCode() {
  return new Promise((resolve) => {
    $.get(
      { url: `http://api.turinglabs.net/api/v1/jd/bean/read/${randomCount}/` },
      (err, resp, data) => {
        try {
          if (err) {
...
function requireConfig() {
  return new Promise((resolve) => {
    ...
    console.log(
      `您提供了${jdPlantBeanShareArr.length}个账号的种豆得豆助力码\n`
    );
    resolve();
  });
}
