...
// [task_local]
// #京东宠汪汪
// 15 */2 * * * https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy.js, tag=京东宠汪汪, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdcww.png, enabled=true
// Loon
// [Script]
// cron "15 */2 * * *" script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy.js,tag=京东宠汪汪
// Surge
// 京东宠汪汪 = type=cron,cronexp="15 */2 * * *",wake-system=1,timeout=20,script-path=https://raw.githubusercontent.com/flatpig/signCollection/master/jd_joy.js
const $ = new Env('宠汪汪');
    ...
    if ($.isNode()) {
      if (process.env.JOY_FEED_COUNT) {
        if ([10, 20, 40, 80].indexOf(process.env.JOY_FEED_COUNT * 1) > -1) {
          FEED_NUM = process.env.JOY_FEED_COUNT
            ? process.env.JOY_FEED_COUNT * 1
            : FEED_NUM;
        } else {
...
async function deskGoodsTask() {
  ...
  if (deskGoodsRes && deskGoodsRes.success) {
    if (deskGoodsRes.data && deskGoodsRes.data.deskGoods) {
      ...
      console.log(
        `浏览货柜商品 ${followCount ? followCount : 0}/${taskChance}`
      );
  ...
  if (`${joyRunFlag}` === 'true') {
    ...
    if ($.petRaceResult) {
      ...
      if (petRaceResult === 'unreceive') {
        ...
        console.log(
          `领取赛跑奖励结果：${JSON.stringify($.receiveJoyRunAwardRes)}`
        );
  ...
  for (let item of $.getPetTaskConfigRes.datas) {
    ...
    if (item['taskType'] === 'SignEveryDay') {
      if (item['receiveStatus'] === 'chance_left') {
        console.log(
          '未完成,需要自己手动去微信小程序【来客有礼】签到，可获得京豆奖励'
        );
    ...
    if (item['taskType'] === 'FollowShop') {
      ...
      for (let shop of followShops) {
        if (!shop.status) {
          ...
          console.log(
            `关注店铺${shop.name}结果::${JSON.stringify(followShopRes)}`
          );
        }
      }
    }
    ...
    if (item['taskType'] === 'ScanMarket') {
      ...
      for (let scanMarketItem of scanMarketList) {
        if (!scanMarketItem.status) {
          ...
          console.log(
            `逛会场-${scanMarketItem.marketName}结果::${JSON.stringify(
              scanMarketRes
            )}`
          );
        }
    ...
    if (item['taskType'] === 'FollowChannel') {
      ...
      for (let followChannelItem of followChannelList) {
        if (!followChannelItem.status) {
          ...
          const scanMarketRes = await scanMarket('scan', body);
          console.log(
            `浏览频道-${followChannelItem.channelName}结果::${JSON.stringify(
              scanMarketRes
            )}`
          );
        }
    ...
    if (item['taskType'] === 'FollowGood') {
      ...
      for (let followGoodItem of followGoodList) {
        if (!followGoodItem.status) {
          ...
          const scanMarketRes = await scanMarket(
            'followGood',
            body,
          ...
          console.log(
            `关注商品-${followGoodItem.skuName}结果::${JSON.stringify(
              scanMarketRes
            )}`
          );
        }
    ...
    if (item['taskType'] === 'ViewVideo') {
      ...
      if (item.taskChance === joinedCount) {
        ...
        for (
          let i = 0;
          i < new Array(item.taskChance - joinedCount).fill('').length;
          i++
        ) {
...
async function appPetTask() {
  ...
  if ($.appGetPetTaskConfigRes.success) {
    for (let item of $.appGetPetTaskConfigRes.datas) {
      if (
        item['taskType'] === 'ScanMarket' &&
        item['receiveStatus'] === 'chance_left'
      ) {
        const scanMarketList = item.scanMarketList;
...
function scanMarket(type, body, cType = 'application/json') {
  return new Promise((resolve) => {
    ...
    $.post(
      taskPostUrl(url, body, reqSource, host, cType),
      (err, resp, data) => {
        try {
          if (err) {
            console.log('\n京东宠汪汪: API查询请求失败 ‼️‼️');
...
function runMatch(teamLevel, timeout = 5000) {
  console.log(
    `正在参赛中，请稍等${timeout / 1000}秒，以防多个账号匹配到统一赛场\n`
  );
